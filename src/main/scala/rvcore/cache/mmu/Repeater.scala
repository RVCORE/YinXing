...
class PTWRepeater(Width: Int = 1)(implicit p: Parameters) extends RVCOREModule with HasPtwConst {
  ...
  RVCOREPerfAccumulate("req_count", ptw.req(0).fire())
  RVCOREPerfAccumulate("tlb_req_cycle", BoolStopWatch(req_in.fire(), tlb.resp.fire() || flush))
  RVCOREPerfAccumulate("ptw_req_cycle", BoolStopWatch(ptw.req(0).fire(), ptw.resp.fire() || flush))

  RVCOREDebug(haveOne, p"haveOne:${haveOne} sent:${sent} recv:${recv} sfence:${flush} req:${req} resp:${resp}")
  RVCOREDebug(req_in.valid || io.tlb.resp.valid, p"tlb: ${tlb}\n")
  RVCOREDebug(io.ptw.req(0).valid || io.ptw.resp.valid, p"ptw: ${ptw}\n")
...
class PTWRepeaterNB(Width: Int = 1, passReady: Boolean = false)(implicit p: Parameters) extends RVCOREModule with HasPtwConst {
  val io = IO(new PTWReapterIO(Width))
  ...
  RVCOREPerfAccumulate("req", req_in.fire())
  RVCOREPerfAccumulate("resp", tlb.resp.fire())
  if (!passReady) {
    RVCOREPerfAccumulate("req_blank", req_in.valid && sent && ptw.req(0).ready)
    RVCOREPerfAccumulate("resp_blank", ptw.resp.valid && recv && tlb.resp.ready)
    RVCOREPerfAccumulate("req_blank_ignore_ready", req_in.valid && sent)
    RVCOREPerfAccumulate("resp_blank_ignore_ready", ptw.resp.valid && recv)
  }
  RVCOREDebug(req_in.valid || io.tlb.resp.valid, p"tlb: ${tlb}\n")
  RVCOREDebug(io.ptw.req(0).valid || io.ptw.resp.valid, p"ptw: ${ptw}\n")
}
...
class PTWFilter(Width: Int, Size: Int)(implicit p: Parameters) extends RVCOREModule with HasPtwConst {
  require(Size >= Width)
  ...
  // perf
  RVCOREPerfAccumulate("tlb_req_count", PopCount(Cat(io.tlb.req.map(_.valid))))
  RVCOREPerfAccumulate("tlb_req_count_filtered", Mux(do_enq, accumEnqNum(Width - 1), 0.U))
  RVCOREPerfAccumulate("ptw_req_count", io.ptw.req(0).fire())
  RVCOREPerfAccumulate("ptw_req_cycle", inflight_counter)
  RVCOREPerfAccumulate("tlb_resp_count", io.tlb.resp.fire())
  RVCOREPerfAccumulate("ptw_resp_count", io.ptw.resp.fire())
  RVCOREPerfAccumulate("inflight_cycle", !isEmptyDeq)
  for (i <- 0 until Size + 1) {
    RVCOREPerfAccumulate(s"counter${i}", counter === i.U)
  }
