...
import rvcore.backend.fu.{PMPReqBundle, PMPConfig}


abstract class TlbBundle(implicit p: Parameters) extends RVCOREBundle with HasTlbConst
abstract class TlbModule(implicit p: Parameters) extends RVCOREModule with HasTlbConst
...
class TlbSPMeta(implicit p: Parameters) extends TlbBundle {
  ...
  def hit(vpn: UInt, asid: UInt): Bool = {
    ...
    RVCOREDebug(Mux(level.asBool, a&b, a), p"Hit superpage: hit:${Mux(level.asBool, a&b, a)} tag:${Hexadecimal(tag)} level:${level} a:${a} b:${b} vpn:${Hexadecimal(vpn)}\n")
