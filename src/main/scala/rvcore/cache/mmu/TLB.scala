...
import rvcore._
import utils._
import rvcore.backend.fu.{PMPChecker, PMPReqBundle}
import rvcore.backend.rob.RobPtr
import rvcore.backend.fu.util.HasCSRConst
...
class TLB(Width: Int, q: TLBParameters)(implicit p: Parameters) extends TlbModule with HasCSRConst with HasPerfEvents {
  ...
  def TLBNormalRead(i: Int) = {
    ...
    RVCOREDebug(validReg, p"(${i.U}) hit:${hit} miss:${miss} ppn:${Hexadecimal(ppn)} perm:${perm}\n")
  ...
  if (!q.shouldBlock) {
    for (i <- 0 until Width) {
      RVCOREPerfAccumulate("first_access" + Integer.toString(i, 10), validRegVec(i) && vmEnable && RegNext(req(i).bits.debug.isFirstIssue))
      RVCOREPerfAccumulate("access" + Integer.toString(i, 10), validRegVec(i) && vmEnable)
    }
    for (i <- 0 until Width) {
      RVCOREPerfAccumulate("first_miss" + Integer.toString(i, 10), validRegVec(i) && vmEnable && missVec(i) && RegNext(req(i).bits.debug.isFirstIssue))
      RVCOREPerfAccumulate("miss" + Integer.toString(i, 10), validRegVec(i) && vmEnable && missVec(i))
    }
  } else {
    // NOTE: ITLB is blocked, so every resp will be valid only when hit
    // every req will be ready only when hit
    for (i <- 0 until Width) {
      RVCOREPerfAccumulate(s"access${i}", io.requestor(i).req.fire() && vmEnable)
      RVCOREPerfAccumulate(s"miss${i}", ptw.req(i).fire())
    }

  }
  //val reqCycleCnt = Reg(UInt(16.W))
  //reqCycleCnt := reqCycleCnt + BoolStopWatch(ptw.req(0).fire(), ptw.resp.fire || sfence.valid)
  //RVCOREPerfAccumulate("ptw_req_count", ptw.req.fire())
  //RVCOREPerfAccumulate("ptw_req_cycle", Mux(ptw.resp.fire(), reqCycleCnt, 0.U))
  RVCOREPerfAccumulate("ptw_resp_count", ptw.resp.fire())
  RVCOREPerfAccumulate("ptw_resp_pf_count", ptw.resp.fire() && ptw.resp.bits.pf)

  // Log
  for(i <- 0 until Width) {
    RVCOREDebug(req(i).valid, p"req(${i.U}): (${req(i).valid} ${req(i).ready}) ${req(i).bits}\n")
    RVCOREDebug(resp(i).valid, p"resp(${i.U}): (${resp(i).valid} ${resp(i).ready}) ${resp(i).bits}\n")
  }

  RVCOREDebug(sfence.valid, p"Sfence: ${sfence}\n")
