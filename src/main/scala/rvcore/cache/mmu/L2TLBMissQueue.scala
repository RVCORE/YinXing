...
class L2TlbMQEntry(implicit p: Parameters) extends RVCOREBundle with HasPtwConst {
...
class L2TlbMQInBundle(implicit p: Parameters) extends RVCOREBundle with HasPtwConst {
...
@chiselName
class L2TlbMissQueue(implicit p: Parameters) extends RVCOREModule with HasPtwConst with HasPerfEvents {
  ...
  RVCOREPerfAccumulate("mq_in_count", io.in.fire())
  RVCOREPerfAccumulate("mq_in_block", io.in.valid && !io.in.ready)
  for (i <- 0 until 7) {
  ...
  for (i <- 0 until (MSHRSize + 1)) {
    ...
    RVCOREPerfAccumulate(s"cache_high_util${i}", PopCount(is_caches_high) === i.U)
    RVCOREPerfAccumulate(s"cache_low_util${i}", PopCount(is_caches_low) === i.U)
