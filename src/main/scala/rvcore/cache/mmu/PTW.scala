...
class PTWImp(outer: PTW)(implicit p: Parameters) extends PtwModule(outer) with HasCSRConst with HasPerfEvents {
  ...
  RVCOREPerfAccumulate(s"req_blocked_by_mq", arb1.io.out.valid && missQueue.io.cache.valid)
...
class PTWWrapper()(implicit p: Parameters) extends LazyModule with HasRVCOREParameter {
