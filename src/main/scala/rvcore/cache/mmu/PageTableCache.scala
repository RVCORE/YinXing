...
import rvcore._
import rvcore.cache.{HasDCacheParameters, MemoryOpConstants}
import utils._
...
@chiselName
class PtwCache()(implicit p: Parameters) extends RVCOREModule with HasPtwConst with HasPerfEvents {
  ...
  val (l1Hit, l1HitPPN, l1Pre) = {
    ...
    for (i <- 0 until l2tlbParams.l1Size) {
      RVCOREDebug(stageReq.fire, p"[l1] l1(${i.U}) ${l1(i)} hit:${l1(i).hit(stageReq.bits.req_info.vpn, io.csr.satp.asid)}\n")
    }
    RVCOREDebug(stageReq.fire, p"[l1] l1v:${Binary(l1v)} hitVecT:${Binary(VecInit(hitVecT).asUInt)}\n")
    RVCOREDebug(stageDelay(0).valid, p"[l1] l1Hit:${hit} l1HitPPN:0x${Hexadecimal(hitPPN)} hitVec:${VecInit(hitVec).asUInt}\n")
  ...
  val (l2Hit, l2HitPPN, l2Pre, l2eccError) = {
    ...
    RVCOREDebug(stageDelay_valid_1cycle, p"[l2] ridx:0x${Hexadecimal(ridx)}\n")
    for (i <- 0 until l2tlbParams.l2nWays) {
      RVCOREDebug(stageCheck_valid_1cycle, p"[l2] ramDatas(${i.U}) ${ramDatas(i)}  l2v:${vVec(i)}  hit:${hit}\n")
    }
    RVCOREDebug(stageCheck_valid_1cycle, p"[l2] l2Hit:${hit} l2HitPPN:0x${Hexadecimal(hitWayData.ppns(genPtwL2SectorIdx(check_vpn)))} hitVec:${Binary(hitVec.asUInt)} hitWay:${hitWay} vidx:${vVec}\n")
  ...
  val (l3Hit, l3HitData, l3Pre, l3eccError) = {
    ...
    RVCOREDebug(stageReq.fire, p"[l3] ridx:0x${Hexadecimal(ridx)}\n")
    for (i <- 0 until l2tlbParams.l3nWays) {
      RVCOREDebug(stageCheck_valid_1cycle, p"[l3] ramDatas(${i.U}) ${ramDatas(i)}  l3v:${vVec(i)}  hit:${hitVec(i)}\n")
    }
    RVCOREDebug(stageCheck_valid_1cycle, p"[l3] l3Hit:${hit} l3HitData:${hitWayData} hitVec:${Binary(hitVec.asUInt)} hitWay:${hitWay} v:${vVec}\n")
  ...
  val (spHit, spHitData, spPre, spValid) = {
    ...
    for (i <- 0 until l2tlbParams.spSize) {
      RVCOREDebug(stageReq.fire, p"[sp] sp(${i.U}) ${sp(i)} hit:${sp(i).hit(stageReq.bits.req_info.vpn, io.csr.satp.asid)} spv:${spv(i)}\n")
    }
    RVCOREDebug(stageDelay_valid_1cycle, p"[sp] spHit:${hit} spHitData:${hitData} hitVec:${Binary(VecInit(hitVec).asUInt)}\n")
  ...
  RVCOREError(stageResp.valid && resp_res.l3.hit && resp_res.sp.hit, "normal page and super page both hit")
  ...
  when (io.refill.valid && !memPte.isPf(refill.level) && !flush ) {
    when (refill.level === 0.U && !memPte.isLeaf()) {
      ...
      RVCOREDebug(p"[l1 refill] refillIdx:${refillIdx} refillEntry:${l1(refillIdx).genPtwEntry(refill.req_info.vpn, io.csr.satp.asid, memSelData, 0.U, prefetch = refill_prefetch)}\n")
      RVCOREDebug(p"[l1 refill] l1v:${Binary(l1v)}->${Binary(l1v | rfOH)} l1g:${Binary(l1g)}->${Binary((l1g & ~rfOH) | Mux(memPte.perm.g, rfOH, 0.U))}\n")
    ...
    when (refill.level === 1.U && !memPte.isLeaf()) {
      ...
      RVCOREDebug(p"[l2 refill] refillIdx:0x${Hexadecimal(refillIdx)} victimWay:${victimWay} victimWayOH:${Binary(victimWayOH)} rfvOH(in UInt):${Cat(refillIdx, victimWay)}\n")
      RVCOREDebug(p"[l2 refill] refilldata:0x${wdata}\n")
      RVCOREDebug(p"[l2 refill] l2v:${Binary(l2v)} -> ${Binary(l2v | rfvOH)}\n")
      RVCOREDebug(p"[l2 refill] l2g:${Binary(l2g)} -> ${Binary(l2g & ~rfvOH | Mux(Cat(memPtes.map(_.perm.g)).andR, rfvOH, 0.U))}\n")
    ...
    when (refill.level === 2.U && memPte.isLeaf()) {
      ...
      RVCOREDebug(p"[l3 refill] refillIdx:0x${Hexadecimal(refillIdx)} victimWay:${victimWay} victimWayOH:${Binary(victimWayOH)} rfvOH(in UInt):${Cat(refillIdx, victimWay)}\n")
      RVCOREDebug(p"[l3 refill] refilldata:0x${wdata}\n")
      RVCOREDebug(p"[l3 refill] l3v:${Binary(l3v)} -> ${Binary(l3v | rfvOH)}\n")
      RVCOREDebug(p"[l3 refill] l3g:${Binary(l3g)} -> ${Binary(l3g & ~rfvOH | Mux(Cat(memPtes.map(_.perm.g)).andR, rfvOH, 0.U))}\n")
  ...
  when (io.refill.valid && !flush && (refill.level === 0.U || refill.level === 1.U) && (memPte.isLeaf() || memPte.isPf(refill.level))) {
    ...
    RVCOREDebug(p"[sp refill] refillIdx:${refillIdx} refillEntry:${sp(refillIdx).genPtwEntry(refill.req_info.vpn, io.csr.satp.asid, memSelData, refill.level, refill_prefetch)}\n")
    RVCOREDebug(p"[sp refill] spv:${Binary(spv)}->${Binary(spv | rfOH)} spg:${Binary(spg)}->${Binary(spg & ~rfOH | Mux(memPte.perm.g, rfOH, 0.U))}\n")
  ...
  RVCOREError(l2eccFlush, "l2tlb.cache.l2 ecc error. Should not happen at sim stage")
  RVCOREError(l3eccFlush, "l2tlb.cache.l3 ecc error. Should not happen at sim stage")
  when (l2eccFlush) {
  ...
  RVCOREPerfAccumulate("access", base_valid_access_0)
  RVCOREPerfAccumulate("l1_hit", base_valid_access_0 && io.resp.bits.toFsm.l1Hit && !io.resp.bits.toFsm.l2Hit && !io.resp.bits.hit)
  RVCOREPerfAccumulate("l2_hit", base_valid_access_0 && io.resp.bits.toFsm.l2Hit && !io.resp.bits.hit)
  RVCOREPerfAccumulate("l3_hit", base_valid_access_0 && resp_l3)
  RVCOREPerfAccumulate("sp_hit", base_valid_access_0 && resp_sp)
  RVCOREPerfAccumulate("pte_hit",base_valid_access_0 && io.resp.bits.hit)

  RVCOREPerfAccumulate("l1_hit_pre", base_valid_access_0 && resp_l1_pre && io.resp.bits.toFsm.l1Hit && !io.resp.bits.toFsm.l2Hit && !io.resp.bits.hit)
  RVCOREPerfAccumulate("l2_hit_pre", base_valid_access_0 && resp_l2_pre && io.resp.bits.toFsm.l2Hit && !io.resp.bits.hit)
  RVCOREPerfAccumulate("l3_hit_pre", base_valid_access_0 && resp_l3_pre && resp_l3)
  RVCOREPerfAccumulate("sp_hit_pre", base_valid_access_0 && resp_sp_pre && resp_sp)
  RVCOREPerfAccumulate("pte_hit_pre",base_valid_access_0 && (resp_l3_pre && resp_l3 || resp_sp_pre && resp_sp) && io.resp.bits.hit)

  val base_valid_access_1 = from_pre(io.resp.bits.req_info.source) && io.resp.fire()
  RVCOREPerfAccumulate("pre_access", base_valid_access_1)
  RVCOREPerfAccumulate("pre_l1_hit", base_valid_access_1 && io.resp.bits.toFsm.l1Hit && !io.resp.bits.toFsm.l2Hit && !io.resp.bits.hit)
  RVCOREPerfAccumulate("pre_l2_hit", base_valid_access_1 && io.resp.bits.toFsm.l2Hit && !io.resp.bits.hit)
  RVCOREPerfAccumulate("pre_l3_hit", base_valid_access_1 && resp_l3)
  RVCOREPerfAccumulate("pre_sp_hit", base_valid_access_1 && resp_sp)
  RVCOREPerfAccumulate("pre_pte_hit",base_valid_access_1 && io.resp.bits.hit)

  RVCOREPerfAccumulate("pre_l1_hit_pre", base_valid_access_1 && resp_l1_pre && io.resp.bits.toFsm.l1Hit && !io.resp.bits.toFsm.l2Hit && !io.resp.bits.hit)
  RVCOREPerfAccumulate("pre_l2_hit_pre", base_valid_access_1 && resp_l2_pre && io.resp.bits.toFsm.l2Hit && !io.resp.bits.hit)
  RVCOREPerfAccumulate("pre_l3_hit_pre", base_valid_access_1 && resp_l3_pre && resp_l3)
  RVCOREPerfAccumulate("pre_sp_hit_pre", base_valid_access_1 && resp_sp_pre && resp_sp)
  RVCOREPerfAccumulate("pre_pte_hit_pre",base_valid_access_1 && (resp_l3_pre && resp_l3 || resp_sp_pre && resp_sp) && io.resp.bits.hit)

  val base_valid_access_2 = stageResp.bits.isFirst && !from_pre(io.resp.bits.req_info.source) && io.resp.fire()
  RVCOREPerfAccumulate("access_first", base_valid_access_2)
  RVCOREPerfAccumulate("l1_hit_first", base_valid_access_2 && io.resp.bits.toFsm.l1Hit && !io.resp.bits.toFsm.l2Hit && !io.resp.bits.hit)
  RVCOREPerfAccumulate("l2_hit_first", base_valid_access_2 && io.resp.bits.toFsm.l2Hit && !io.resp.bits.hit)
  RVCOREPerfAccumulate("l3_hit_first", base_valid_access_2 && resp_l3)
  RVCOREPerfAccumulate("sp_hit_first", base_valid_access_2 && resp_sp)
  RVCOREPerfAccumulate("pte_hit_first",base_valid_access_2 && io.resp.bits.hit)

  RVCOREPerfAccumulate("l1_hit_pre_first", base_valid_access_2 && resp_l1_pre && io.resp.bits.toFsm.l1Hit && !io.resp.bits.toFsm.l2Hit && !io.resp.bits.hit)
  RVCOREPerfAccumulate("l2_hit_pre_first", base_valid_access_2 && resp_l2_pre && io.resp.bits.toFsm.l2Hit && !io.resp.bits.hit)
  RVCOREPerfAccumulate("l3_hit_pre_first", base_valid_access_2 && resp_l3_pre && resp_l3)
  RVCOREPerfAccumulate("sp_hit_pre_first", base_valid_access_2 && resp_sp_pre && resp_sp)
  RVCOREPerfAccumulate("pte_hit_pre_first",base_valid_access_2 && (resp_l3_pre && resp_l3 || resp_sp_pre && resp_sp) && io.resp.bits.hit)

  val base_valid_access_3 = stageResp.bits.isFirst && from_pre(io.resp.bits.req_info.source) && io.resp.fire()
  RVCOREPerfAccumulate("pre_access_first", base_valid_access_3)
  RVCOREPerfAccumulate("pre_l1_hit_first", base_valid_access_3 && io.resp.bits.toFsm.l1Hit && !io.resp.bits.toFsm.l2Hit && !io.resp.bits.hit)
  RVCOREPerfAccumulate("pre_l2_hit_first", base_valid_access_3 && io.resp.bits.toFsm.l2Hit && !io.resp.bits.hit)
  RVCOREPerfAccumulate("pre_l3_hit_first", base_valid_access_3 && resp_l3)
  RVCOREPerfAccumulate("pre_sp_hit_first", base_valid_access_3 && resp_sp)
  RVCOREPerfAccumulate("pre_pte_hit_first", base_valid_access_3 && io.resp.bits.hit)

  RVCOREPerfAccumulate("pre_l1_hit_pre_first", base_valid_access_3 && resp_l1_pre && io.resp.bits.toFsm.l1Hit && !io.resp.bits.toFsm.l2Hit && !io.resp.bits.hit)
  RVCOREPerfAccumulate("pre_l2_hit_pre_first", base_valid_access_3 && resp_l2_pre && io.resp.bits.toFsm.l2Hit && !io.resp.bits.hit)
  RVCOREPerfAccumulate("pre_l3_hit_pre_first", base_valid_access_3 && resp_l3_pre && resp_l3)
  RVCOREPerfAccumulate("pre_sp_hit_pre_first", base_valid_access_3 && resp_sp_pre && resp_sp)
  RVCOREPerfAccumulate("pre_pte_hit_pre_first",base_valid_access_3 && (resp_l3_pre && resp_l3 || resp_sp_pre && resp_sp) && io.resp.bits.hit)

  RVCOREPerfAccumulate("rwHarzad", io.req.valid && !io.req.ready)
  RVCOREPerfAccumulate("out_blocked", io.resp.valid && !io.resp.ready)
  l1AccessPerf.zipWithIndex.map{ case (l, i) => RVCOREPerfAccumulate(s"L1AccessIndex${i}", l) }
  l2AccessPerf.zipWithIndex.map{ case (l, i) => RVCOREPerfAccumulate(s"L2AccessIndex${i}", l) }
  l3AccessPerf.zipWithIndex.map{ case (l, i) => RVCOREPerfAccumulate(s"L3AccessIndex${i}", l) }
  spAccessPerf.zipWithIndex.map{ case (l, i) => RVCOREPerfAccumulate(s"SPAccessIndex${i}", l) }
  l1RefillPerf.zipWithIndex.map{ case (l, i) => RVCOREPerfAccumulate(s"L1RefillIndex${i}", l) }
  l2RefillPerf.zipWithIndex.map{ case (l, i) => RVCOREPerfAccumulate(s"L2RefillIndex${i}", l) }
  l3RefillPerf.zipWithIndex.map{ case (l, i) => RVCOREPerfAccumulate(s"L3RefillIndex${i}", l) }
  spRefillPerf.zipWithIndex.map{ case (l, i) => RVCOREPerfAccumulate(s"SPRefillIndex${i}", l) }

  RVCOREPerfAccumulate("l1Refill", Cat(l1RefillPerf).orR)
  RVCOREPerfAccumulate("l2Refill", Cat(l2RefillPerf).orR)
  RVCOREPerfAccumulate("l3Refill", Cat(l3RefillPerf).orR)
  RVCOREPerfAccumulate("spRefill", Cat(spRefillPerf).orR)
  RVCOREPerfAccumulate("l1Refill_pre", Cat(l1RefillPerf).orR && refill_prefetch)
  RVCOREPerfAccumulate("l2Refill_pre", Cat(l2RefillPerf).orR && refill_prefetch)
  RVCOREPerfAccumulate("l3Refill_pre", Cat(l3RefillPerf).orR && refill_prefetch)
  RVCOREPerfAccumulate("spRefill_pre", Cat(spRefillPerf).orR && refill_prefetch)

  // debug
  RVCOREDebug(sfence.valid, p"[sfence] original v and g vector:\n")
  RVCOREDebug(sfence.valid, p"[sfence] l1v:${Binary(l1v)}\n")
  RVCOREDebug(sfence.valid, p"[sfence] l2v:${Binary(l2v)}\n")
  RVCOREDebug(sfence.valid, p"[sfence] l3v:${Binary(l3v)}\n")
  RVCOREDebug(sfence.valid, p"[sfence] l3g:${Binary(l3g)}\n")
  RVCOREDebug(sfence.valid, p"[sfence] spv:${Binary(spv)}\n")
  RVCOREDebug(RegNext(sfence.valid), p"[sfence] new v and g vector:\n")
  RVCOREDebug(RegNext(sfence.valid), p"[sfence] l1v:${Binary(l1v)}\n")
  RVCOREDebug(RegNext(sfence.valid), p"[sfence] l2v:${Binary(l2v)}\n")
  RVCOREDebug(RegNext(sfence.valid), p"[sfence] l3v:${Binary(l3v)}\n")
  RVCOREDebug(RegNext(sfence.valid), p"[sfence] l3g:${Binary(l3g)}\n")
  RVCOREDebug(RegNext(sfence.valid), p"[sfence] spv:${Binary(spv)}\n")
