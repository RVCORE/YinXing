...
class TLBFA(
  ...
  RVCOREPerfAccumulate(s"access", io.r.resp.map(_.valid.asUInt()).fold(0.U)(_ + _))
  RVCOREPerfAccumulate(s"hit", io.r.resp.map(a => a.valid && a.bits.hit).fold(0.U)(_.asUInt() + _.asUInt()))

  for (i <- 0 until nWays) {
    RVCOREPerfAccumulate(s"access${i}", io.r.resp.zip(io.access.map(acc => UIntToOH(acc.touch_ways.bits))).map{ case (a, b) =>
      a.valid && a.bits.hit && b(i)}.fold(0.U)(_.asUInt() + _.asUInt()))
  }
  for (i <- 0 until nWays) {
    RVCOREPerfAccumulate(s"refill${i}", io.w.valid && io.w.bits.wayIdx === i.U)
  }
...
class TLBSA(
  ...
  RVCOREPerfAccumulate(s"access", io.r.req.map(_.valid.asUInt()).fold(0.U)(_ + _))
  RVCOREPerfAccumulate(s"hit", io.r.resp.map(a => a.valid && a.bits.hit).fold(0.U)(_.asUInt() + _.asUInt()))

  for (i <- 0 until nSets) {
    for (j <- 0 until nWays) {
      RVCOREPerfAccumulate(s"refill${i}_${j}", (io.w.valid || io.victim.in.valid) &&
  ...
  for (i <- 0 until nSets) {
    for (j <- 0 until nWays) {
      RVCOREPerfAccumulate(s"hit${i}_${j}", io.r.resp.map(_.valid)
  ...
  for (i <- 0 until nSets) {
    RVCOREPerfAccumulate(s"access${i}", io.r.resp.map(_.valid)
      .zip(io.r.req.map(a => RegNext(get_set_idx(a.bits.vpn, nSets)) === i.U))
      .map{a => (a._1 && a._2).asUInt()}
      .fold(0.U)(_ + _)
    )
