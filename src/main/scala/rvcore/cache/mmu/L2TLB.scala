...
import rvcore.backend.fu.{PMP, PMPChecker, PMPReqBundle, PMPRespBundle}
...
class L2TLBImp(outer: L2TLB)(implicit p: Parameters) extends PtwModule(outer) with HasCSRConst with HasPerfEvents {
  ...
  // debug info
  for (i <- 0 until PtwWidth) {
    RVCOREDebug(p"[io.tlb(${i.U})] ${io.tlb(i)}\n")
  }
  RVCOREDebug(p"[sfence] ${sfence}\n")
  RVCOREDebug(p"[io.csr.tlb] ${io.csr.tlb}\n")

  for (i <- 0 until PtwWidth) {
    RVCOREPerfAccumulate(s"req_count${i}", io.tlb(i).req(0).fire())
    RVCOREPerfAccumulate(s"req_blocked_count_${i}", io.tlb(i).req(0).valid && !io.tlb(i).req(0).ready)
  }
  RVCOREPerfAccumulate(s"req_blocked_by_mq", arb1.io.out.valid && missQueue.io.out.valid)
  for (i <- 0 until (MemReqWidth + 1)) {
    RVCOREPerfAccumulate(s"mem_req_util${i}", PopCount(waiting_resp) === i.U)
  }
  RVCOREPerfAccumulate("mem_cycle", PopCount(waiting_resp) =/= 0.U)
  RVCOREPerfAccumulate("mem_count", mem.a.fire())
...
class FakePTW()(implicit p: Parameters) extends RVCOREModule with HasPtwConst {
  val io = IO(new L2TLBIO)
...
class L2TLBWrapper()(implicit p: Parameters) extends LazyModule with HasRVCOREParameter {
