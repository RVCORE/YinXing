...
trait MemoryOpConstants {
  ...
  def isRead(cmd: UInt) = cmd === M_XRD || cmd === M_XLR || cmd === M_RVCOREC || isAMO(cmd)
  def isWrite(cmd: UInt) = cmd === M_XWR || cmd === M_PWR || cmd === M_RVCOREC || isAMO(cmd)
