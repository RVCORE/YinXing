...
import rvcore.cache.mmu.{HasTlbConst}
import utils._
...
class StreamBuffer(implicit p: Parameters) extends PrefetchModule with HasTlbConst {
  ...
  when (io.update.valid && !empty && (isPrefetching(hitIdx) || valid(hitIdx))) {
    ...
    RVCOREDebug(io.update.valid && !empty && (isPrefetching(hitIdx) || valid(hitIdx)), p"hitIdx=${hitIdx} headBeforehitIdx=${headBeforehitIdx} hitIdxBeforeHead=${hitIdxBeforeHead}\n")
  }
  ...
  when (!full && baseReq.valid && !needRealloc) {
    when (!nextAddrCrossPage) {
      ...
      RVCOREDebug(p"enqueue 0x${nextReq.addr}\n")
    }.otherwise {
      RVCOREDebug(p"addr 0x${nextReq.addr} could not enqueue for crossing pages\n")
    }
  }
  ...
  // debug info
  RVCOREDebug(s"${streamParams.cacheName} " + p"StreamBuf ${io.streamBufId} io.req:    v=${io.req.valid} r=${io.req.ready} ${io.req.bits}\n")
  RVCOREDebug(s"${streamParams.cacheName} " + p"StreamBuf ${io.streamBufId} io.resp:   v=${io.resp.valid} r=${io.resp.ready} ${io.resp.bits}\n")
  RVCOREDebug(s"${streamParams.cacheName} " + p"StreamBuf ${io.streamBufId} io.finish: v=${io.finish.valid} r=${io.finish.ready} ${io.finish.bits}\n")
  RVCOREDebug(s"${streamParams.cacheName} " + p"StreamBuf ${io.streamBufId} io.update: v=${io.update.valid} ${io.update.bits}\n")
  RVCOREDebug(s"${streamParams.cacheName} " + p"StreamBuf ${io.streamBufId} io.alloc:  v=${io.alloc.valid} ${io.alloc.bits}\n")
  for (i <- 0 until streamSize) {
    RVCOREDebug(s"${streamParams.cacheName} " + p"StreamBuf ${io.streamBufId} [${i.U}] io.addrs: ${io.addrs(i).valid} 0x${Hexadecimal(io.addrs(i).bits)} " +
      p"buf: ${buf(i)} valid: ${valid(i)} state: ${state(i)} isPfting: ${isPrefetching(i)} " +
      p"deqLater: ${deqLater(i)} deqValid: ${deqValid(i)}\n")
  }
  RVCOREDebug(s"${streamParams.cacheName} " + p"StreamBuf ${io.streamBufId} head: ${head} tail: ${tail} full: ${full} empty: ${empty} nextHead: ${nextHead} blockBytes: ${blockBytes.U}\n")
  RVCOREDebug(s"${streamParams.cacheName} " + p"StreamBuf ${io.streamBufId} baseReq: v=${baseReq.valid} ${baseReq.bits} nextReq: ${nextReq} crossPage: ${nextAddrCrossPage}\n")
  RVCOREDebug(needRealloc, s"${streamParams.cacheName} " + p"StreamBuf ${io.streamBufId} needRealloc: ${needRealloc} reallocReq: ${reallocReq}\n")
  RVCOREDebug(s"${streamParams.cacheName} " + p"StreamBuf ${io.streamBufId} prefetchPrior: ")
  (0 until streamSize).foreach(i => RVCOREDebug(false, true.B, p"${prefetchPrior(i)} "))
...
class StreamPrefetch(implicit p: Parameters) extends PrefetchModule {
  ...
  // debug info
  RVCOREDebug(s"${streamParams.cacheName} " + p"io: ${io}\n")
  RVCOREDebug(s"${streamParams.cacheName} " + p"bufValids: ${Binary(bufValids.asUInt)} hit: ${hit} ages: ")
  (0 until streamCnt).foreach(i => RVCOREDebug(false, true.B, p"${Hexadecimal(ages(i))} "))
  RVCOREDebug(false, true.B, "\n")
}
