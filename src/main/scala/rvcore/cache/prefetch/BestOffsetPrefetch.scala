...
class RecentRequestTable(implicit p: Parameters) extends PrefetchModule {
  ...
  // debug info
  RVCOREDebug(io.w.fire(), p"io.write: v=${io.w.valid} addr=0x${Hexadecimal(io.w.bits)}\n")
  RVCOREDebug(p"io.read: ${io.r}\n")
  RVCOREDebug(io.w.fire(), p"wAddr=0x${Hexadecimal(wAddr)} idx=${Hexadecimal(idx(wAddr))} tag=${Hexadecimal(tag(wAddr))}\n")
  RVCOREDebug(io.r.req.fire(), p"rAddr=0x${Hexadecimal(rAddr)} idx=${Hexadecimal(idx(rAddr))} rData=${rData}\n")
...
class OffsetScoreTable(implicit p: Parameters) extends PrefetchModule {
  ...
  when (state === s_learn) {
    when (io.test.req.fire()) {
      ...
      RVCOREDebug(p"test offset ${testOffset} req fire\n")
    }

    // (2) the number of rounds equals ROUNDMAX.
    when (round >= roundMax.U) {
      state := s_idle
      RVCOREDebug(p"round reaches roundMax(${roundMax.U})\n")
    }
    ...
    when (io.test.resp.fire() && io.test.resp.bits.hit) {
      ...
      when (newScore >= scoreMax.U) {
        state := s_idle
        RVCOREDebug(p"newScore reaches scoreMax(${scoreMax.U})\n")
      }

      RVCOREDebug(p"test offset ${offset} resp fire and hit. score ${oldScore} -> ${newScore}\n")
    }
  }
  ...
  RVCOREDebug(p"state=${state} prefetchOffset=${prefetchOffset} ptr=${ptr} round=${round} bestOffset=${bestOffset} testOffset=${testOffset}\n")
  // score table
  RVCOREDebug(p"OffsetScoreTable(idx:offset:score) as follows:\n")
  for (i <- 0 until scores) {
    if (i % 8 == 0) { RVCOREDebug(p"${i.U}:${st(i)}\t") }
    else if (i % 8 == 7 || i == scores - 1) { RVCOREDebug(false, true.B, p"${i.U}:${st(i)}\n") }
    else { RVCOREDebug(false, true.B, p"${i.U}:${st(i)}\t") }
  }
  RVCOREDebug(io.req.fire(), p"receive req from L1. io.req.bits=0x${Hexadecimal(io.req.bits)}\n")
}
...
class BestOffsetPrefetchEntry(implicit p: Parameters) extends PrefetchModule with HasTlbConst {
  ...
  when (state === s_idle) {
    when (io.pft.train.valid) {
      ...
      RVCOREDebug(crossPage, p"prefetch addr 0x${nextAddr} cross page, ignore this!\n")
    }
  }
  ...
  RVCOREDebug(p"bopEntry ${io.id}: state=${state} prefetchOffset=${io.prefetchOffset} inflight=${io.inflight.valid} 0x${Hexadecimal(io.inflight.bits)} writeRRTable: ${io.writeRRTable.valid} 0x${Hexadecimal(io.writeRRTable.bits)} baseAddr=0x${Hexadecimal(baseAddr)} nextAddr=0x${Hexadecimal(nextAddr)} crossPage=${crossPage} req: ${req}\n")
  RVCOREDebug(p"bopEntry ${io.id}: io.pft: ${io.pft}\n")
}
...
class BestOffsetPrefetch(implicit p: Parameters) extends PrefetchModule {
  ...
  RVCOREDebug(p"io: ${io}\n")
  RVCOREDebug(p"entryReadyIdx=${entryReadyIdx} inflightMatchVec=${Binary(inflightMatchVec.asUInt)}\n")
