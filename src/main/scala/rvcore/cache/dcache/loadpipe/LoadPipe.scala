...
import utils.{HasPerfEvents, RVCOREDebug, RVCOREPerfAccumulate}
import rvcore.L1CacheErrorInfo
...
//  s1_fake_meta.tag := get_tag(s1_addr)
  ...
  RVCOREPerfAccumulate("dcache_read_bank_conflict", io.bank_conflict_slow && s2_valid)
  ...
  def dump_pipeline_reqs(pipeline_stage_name: String, valid: Bool,
    req: DCacheWordReq ) = {
      when (valid) {
        RVCOREDebug(s"$pipeline_stage_name: ")
        req.dump()
      }
  }
  ...
  def dump_pipeline_valids(pipeline_stage_name: String, signal_name: String, valid: Bool) = {
    when (valid) {
      RVCOREDebug(s"$pipeline_stage_name $signal_name\n")
    }
  }

  // performance counters
  RVCOREPerfAccumulate("load_req", io.lsu.req.fire())
  RVCOREPerfAccumulate("load_s1_kill", s1_fire && io.lsu.s1_kill)
  RVCOREPerfAccumulate("load_hit_way", s1_fire && s1_tag_match)
  RVCOREPerfAccumulate("load_replay", io.lsu.resp.fire() && resp.bits.replay)
  RVCOREPerfAccumulate("load_replay_for_data_nack", io.lsu.resp.fire() && resp.bits.replay && s2_nack_data)
  RVCOREPerfAccumulate("load_replay_for_no_mshr", io.lsu.resp.fire() && resp.bits.replay && s2_nack_no_mshr)
  RVCOREPerfAccumulate("load_replay_for_conflict", io.lsu.resp.fire() && resp.bits.replay && io.bank_conflict_slow)
  RVCOREPerfAccumulate("load_hit", io.lsu.resp.fire() && !real_miss)
  RVCOREPerfAccumulate("load_miss", io.lsu.resp.fire() && real_miss)
  RVCOREPerfAccumulate("load_succeed", io.lsu.resp.fire() && !resp.bits.miss && !resp.bits.replay)
  RVCOREPerfAccumulate("load_miss_or_conflict", io.lsu.resp.fire() && resp.bits.miss)
  RVCOREPerfAccumulate("actual_ld_fast_wakeup", s1_fire && s1_tag_match && !io.disable_ld_fast_wakeup)
  RVCOREPerfAccumulate("ideal_ld_fast_wakeup", io.banked_data_read.fire() && s1_tag_match)
