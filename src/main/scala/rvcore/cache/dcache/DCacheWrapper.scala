...
class DCacheWordReq(implicit p: Parameters)  extends DCacheBundle
{
  ...
  def dump() = {
    RVCOREDebug("DCacheWordReq: cmd: %x addr: %x data: %x mask: %x id: %d\n",
...
class DCacheLineReq(implicit p: Parameters)  extends DCacheBundle
{
  ...
  val id     = UInt(reqIdWidth.W)
  def dump() = {
    RVCOREDebug("DCacheLineReq: cmd: %x addr: %x data: %x mask: %x id: %d\n",
      cmd, addr, data, mask, id)
  }
...
class DCacheWordResp(implicit p: Parameters) extends DCacheBundle
{
  ...
  def dump() = {
    RVCOREDebug("DCacheWordResp: data: %x id: %d miss: %b replay: %b\n",
      data, id, miss, replay)
  }
}
...
class DCacheLineResp(implicit p: Parameters) extends DCacheBundle
{
  ...
  val id     = UInt(reqIdWidth.W)
  def dump() = {
    RVCOREDebug("DCacheLineResp: data: %x id: %d miss: %b replay: %b\n",
      data, id, miss, replay)
  }
}
...
class Refill(implicit p: Parameters) extends DCacheBundle
{
  ...
  def dump() = {
    RVCOREDebug("Refill: addr: %x data: %x\n", addr, data)
  }
}
...
class DCacheImp(outer: DCache) extends LazyModuleImp(outer) with HasDCacheParameters with HasPerfEvents {
  ...
  RVCOREPerfAccumulate("num_loads", num_loads)
  ...
  RVCOREPerfAccumulate("access_early_replace", PopCount(Cat(access_early_replace)))
...
class DCacheWrapper()(implicit p: Parameters) extends LazyModule with HasRVCOREParameter {
