...
import utils.{HasPerfEvents, HasTLDump, RVCOREDebug, RVCOREPerfAccumulate}
...
class WritebackReq(implicit p: Parameters) extends DCacheBundle {
  ...
  def dump() = {
    RVCOREDebug("WritebackReq addr: %x param: %d voluntary: %b hasData: %b data: %x\n",
...
class WritebackEntry(edge: TLEdgeOut)(implicit p: Parameters) extends DCacheModule with HasTLDump
{
  ...
  when (state =/= s_invalid) {
    RVCOREDebug("WritebackEntry: %d state: %d block_addr: %x\n", io.id, state, io.block_addr.bits)
  }
...
//          release_later := false.B
  ...
  RVCOREPerfAccumulate("wb_req", io.req.fire())
  RVCOREPerfAccumulate("wb_release", state === s_release_req && release_done && req.voluntary)
  RVCOREPerfAccumulate("wb_probe_resp", state === s_release_req && release_done && !req.voluntary)
  RVCOREPerfAccumulate("penalty_blocked_by_channel_C", io.mem_release.valid && !io.mem_release.ready)
  RVCOREPerfAccumulate("penalty_waiting_for_channel_D", io.mem_grant.ready && !io.mem_grant.valid && state === s_release_resp)
}
...
//      }
  ...
  when (io.miss_req.valid) {
    RVCOREDebug("miss_req: addr: %x\n", io.miss_req.bits)
  }

  when (io.block_miss_req) {
    RVCOREDebug("block_miss_req\n")
  }
