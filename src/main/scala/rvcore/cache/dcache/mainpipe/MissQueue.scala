...
class MissEntry(edge: TLEdgeOut)(implicit p: Parameters) extends DCacheModule {
  ...
  RVCOREPerfAccumulate("miss_req_primary", primary_fire)
  RVCOREPerfAccumulate("miss_req_merged", secondary_fire)
  RVCOREPerfAccumulate("load_miss_penalty_to_use",
    should_refill_data &&
      BoolStopWatch(primary_fire, io.refill_to_ldq.valid, true)
  )
  RVCOREPerfAccumulate("main_pipe_penalty", BoolStopWatch(io.main_pipe_req.fire(), io.main_pipe_resp))
  RVCOREPerfAccumulate("penalty_blocked_by_channel_A", io.mem_acquire.valid && !io.mem_acquire.ready)
  RVCOREPerfAccumulate("penalty_waiting_for_channel_D", s_acquire && !w_grantlast && !io.mem_grant.valid)
  RVCOREPerfAccumulate("penalty_waiting_for_channel_E", io.mem_finish.valid && !io.mem_finish.ready)
  RVCOREPerfAccumulate("penalty_from_grant_to_refill", !w_refill_resp && w_grantlast)
  RVCOREPerfAccumulate("soft_prefetch_number", primary_fire && io.req.bits.source === SOFT_PREFETCH.U)

  val (mshr_penalty_sample, mshr_penalty) = TransactionLatencyCounter(RegNext(primary_fire), release_entry)
  RVCOREPerfHistogram("miss_penalty", mshr_penalty, mshr_penalty_sample, 0, 20, 1, true, true)
  RVCOREPerfHistogram("miss_penalty", mshr_penalty, mshr_penalty_sample, 20, 100, 10, true, false)
  ...
  RVCOREPerfHistogram("load_miss_penalty_to_use", load_miss_penalty, load_miss_penalty_sample, 0, 20, 1, true, true)
  RVCOREPerfHistogram("load_miss_penalty_to_use", load_miss_penalty, load_miss_penalty_sample, 20, 100, 10, true, false)

  val (a_to_d_penalty_sample, a_to_d_penalty) = TransactionLatencyCounter(io.mem_acquire.fire(), io.mem_grant.fire() && refill_done)
  RVCOREPerfHistogram("a_to_d_penalty", a_to_d_penalty, a_to_d_penalty_sample, 0, 20, 1, true, true)
  RVCOREPerfHistogram("a_to_d_penalty", a_to_d_penalty, a_to_d_penalty_sample, 20, 100, 10, true, false)
}
...
//  assert(RegNext(PopCount(Seq(merge, reject)) <= 1.U))
  ...
  RVCOREPerfAccumulate("miss_req", io.req.fire())
  RVCOREPerfAccumulate("miss_req_allocate", io.req.fire() && alloc)
  RVCOREPerfAccumulate("miss_req_merge_load", io.req.fire() && merge && io.req.bits.isLoad)
  RVCOREPerfAccumulate("miss_req_reject_load", io.req.valid && reject && io.req.bits.isLoad)
  RVCOREPerfAccumulate("probe_blocked_by_miss", io.probe_block)
  ...
  RVCOREPerfAccumulate("max_inflight", max_inflight)
  QueuePerf(cfg.nMissEntries, num_valids, num_valids === cfg.nMissEntries.U)
  io.full := num_valids === cfg.nMissEntries.U
  RVCOREPerfHistogram("num_valids", num_valids, true.B, 0, cfg.nMissEntries, 1)
