...
class ProbeReq(implicit p: Parameters) extends DCacheBundle
{
  ...
  def dump() = {
    RVCOREDebug("ProbeReq source: %d opcode: %d addr: %x param: %d\n",
...
class ProbeEntry(implicit p: Parameters) extends DCacheModule {
  ...
  when (state =/= s_invalid) {
    RVCOREDebug("ProbeEntry: state: %d block_addr: %x\n", state, io.block_addr.bits)
  }
  ...
  RVCOREPerfAccumulate("probe_req", state === s_invalid && io.req.fire())
  RVCOREPerfAccumulate("probe_penalty", state =/= s_invalid)
  RVCOREPerfAccumulate("probe_penalty_blocked_by_lrsc", state === s_pipe_req && io.lrsc_locked_block.valid && get_block(io.lrsc_locked_block.bits) === get_block(req.addr))
  RVCOREPerfAccumulate("probe_penalty_blocked_by_pipeline", state === s_pipe_req && io.pipe_req.valid && !io.pipe_req.ready)
}
...
//  }
  ...
  when (io.lrsc_locked_block.valid) {
    RVCOREDebug("lrsc_locked_block: %x\n", io.lrsc_locked_block.bits)
  }
