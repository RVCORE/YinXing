...
class MainPipe(implicit p: Parameters) extends DCacheModule with HasPerfEvents {
  ...
  val amo_wait_amoalu = s3_req.isAMO && s3_req.cmd =/= M_XLR && s3_req.cmd =/= M_RVCOREC
  ...
  val s3_sc = !s3_req.probe && s3_req.isAMO && s3_req.cmd === M_RVCOREC
  ...
  when (s3_valid) {
    when (s3_req.addr === debug_sc_fail_addr) {
      ...
      when (s3_sc_fail) {
        ...
        RVCOREWarn(s3_sc_fail === 100.U, p"L1DCache failed too many SCs in a row 0x${Hexadecimal(debug_sc_fail_addr)}, check if sth went wrong\n")
      }
    }
  }
