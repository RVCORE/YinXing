...
import utils.{Code, ParallelOR, ReplacementPolicy, SRAMTemplate, RVCOREDebug, RVCOREPerfAccumulate}
...
abstract class AbstractBankedDataArray(implicit p: Parameters) extends DCacheModule
{
  ...
  def dumpRead() = {
    (0 until LoadPipelineWidth) map { w =>
      when(io.read(w).valid) {
        RVCOREDebug(s"DataArray Read channel: $w valid way_en: %x addr: %x\n",
          io.read(w).bits.way_en, io.read(w).bits.addr)
      }
    }
    when(io.readline.valid) {
      RVCOREDebug(s"DataArray Read Line, valid way_en: %x addr: %x rmask %x\n",
  ...
  def dumpWrite() = {
    when(io.write.valid) {
      RVCOREDebug(s"DataArray Write valid way_en: %x addr: %x\n",
        io.write.bits.way_en, io.write.bits.addr)

      (0 until DCacheBanks) map { r =>
        RVCOREDebug(s"cycle: $r data: %x wmask: %x\n",
  ...
  def dumpResp() = {
    RVCOREDebug(s"DataArray ReadeResp channel:\n")
    (0 until DCacheBanks) map { r =>
      RVCOREDebug(s"cycle: $r data: %x\n", io.resp(r).raw_data)
    }
  }
...
class BankedDataArray(implicit p: Parameters) extends AbstractBankedDataArray {
  ...
  class DataSRAMBank(index: Int) extends Module {
    ...
    def dump_r() = {
      when(RegNext(io.r.en)) {
        RVCOREDebug("bank read addr %x way_en %x data %x\n",
          RegNext(io.r.addr),
          RegNext(io.r.way_en),
          io.r.data
        )
    ...
    def dump_w() = {
      when(io.w.en) {
        RVCOREDebug("bank write addr %x way_en %x data %x\n",
          io.w.addr,
          io.w.way_en,
          io.w.data
        )
  ...
  RVCOREPerfAccumulate("data_array_multi_read", perf_multi_read)
  (1 until LoadPipelineWidth).foreach(y => (0 until y).foreach(x =>
    RVCOREPerfAccumulate(s"data_array_rr_bank_conflict_${x}_${y}", rr_bank_conflict(x)(y))
  ))
  (0 until LoadPipelineWidth).foreach(i => {
    RVCOREPerfAccumulate(s"data_array_rrl_bank_conflict_${i}", rrl_bank_conflict(i))
    RVCOREPerfAccumulate(s"data_array_rw_bank_conflict_${i}", rw_bank_conflict(i))
    RVCOREPerfAccumulate(s"data_array_read_${i}", io.read(i).valid)
  })
  RVCOREPerfAccumulate("data_array_access_total", PopCount(io.read.map(_.valid)))
  RVCOREPerfAccumulate("data_array_read_line", io.readline.valid)
  RVCOREPerfAccumulate("data_array_write", io.write.valid)
  ...
  for (bank_index <- 0 until DCacheBanks) {
    ...
    when(ecc_bank.io.w.req.valid) {
      RVCOREDebug("write in ecc sram: bank %x set %x data %x waymask %x\n",
        bank_index.U,
        sram_waddr,
