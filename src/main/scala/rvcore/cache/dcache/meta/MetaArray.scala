...
import utils.{Code, ParallelOR, ReplacementPolicy, SRAMTemplate, RVCOREDebug}
...
class L1MetadataArray(onReset: () => L1Metadata)(implicit p: Parameters) extends DCacheModule {
  ...
  def dumpRead() = {
    when(io.read.fire()) {
      RVCOREDebug("MetaArray Read: idx: %d way_en: %x tag: %x\n",
  ...
  def dumpWrite() = {
    when(io.write.fire()) {
      RVCOREDebug("MetaArray Write: idx: %d way_en: %x tag: %x new_tag: %x new_coh: %x\n",
  ...
  // def dumpResp() = {
  //   (0 until nWays) map { i =>
  //     RVCOREDebug(s"MetaArray Resp: way: $i tag: %x coh: %x\n",
...
class DuplicatedMetaArray(numReadPorts: Int)(implicit p: Parameters) extends DCacheModule {
  ...
  def dumpRead() = {
    (0 until numReadPorts) map { w =>
      when(io.read(w).fire()) {
        RVCOREDebug(s"MetaArray Read channel: $w idx: %d way_en: %x tag: %x\n",
  ...
  //     (0 until nWays) map { i =>
  //       RVCOREDebug(s"MetaArray Resp: channel: $w way: $i tag: %x coh: %x\n",
