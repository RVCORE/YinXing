...
trait BimParams extends HasRVCOREParameter {
  val bimSize = 2048
  val bypassEntries = 4
}
...
class BIM(implicit p: Parameters) extends BasePredictor with BimParams with BPUUtils {
  ...
  RVCOREDebug(doing_reset, "Doing reset...\n")

  RVCOREDebug(io.s0_fire, "req_pc=%x, req_idx=%d\n", s0_pc, s0_idx)

  for(i <- 0 until numBr) {
    RVCOREDebug(latch_s0_fire, "last_cycle req %d: ctr=%b\n", i.U, s1_read(i))
  }

  RVCOREDebug(u_valid, "update_pc=%x, update_idx=%d, is_br=%b\n", update.pc, u_idx, update.ftb_entry.brValids.asUInt)

  RVCOREDebug(u_valid, "newTakens=%b\n", newTakens.asUInt)

  for(i <- 0 until numBr) {
    RVCOREDebug(u_valid, "oldCtrs%d=%b\n", i.U, oldCtrs(i))
  }

  for(i <- 0 until numBr) {
    RVCOREDebug(u_valid, "newCtrs%d=%b\n", i.U, newCtrs(i))
  }
