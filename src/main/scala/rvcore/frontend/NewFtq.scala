...
import utils._
import rvcore._
import rvcore.frontend.icache._
import rvcore.backend.CtrlToFtqIO
import rvcore.backend.decode.ImmUnion

class FtqPtr(implicit p: Parameters) extends CircularQueuePtr[FtqPtr](
  p => p(RVCORECoreParamsKey).FtqSize
){
}
...
class FtqNRSRAM[T <: Data](gen: T, numRead: Int)(implicit p: Parameters) extends RVCOREModule {
...
class Ftq_RF_Components(implicit p: Parameters) extends RVCOREBundle with BPUUtils {
...
class Ftq_pd_Entry(implicit p: Parameters) extends RVCOREBundle {
...
class Ftq_Redirect_SRAMEntry(implicit p: Parameters) extends RVCOREBundle with HasBPUConst {
...
class Ftq_1R_SRAMEntry(implicit p: Parameters) extends RVCOREBundle with HasBPUConst {
  val meta = UInt(MaxMetaLength.W)
}

class Ftq_Pred_Info(implicit p: Parameters) extends RVCOREBundle {
  val target = UInt(VAddrBits.W)
  val cfiIndex = ValidUndirectioned(UInt(log2Ceil(PredictWidth).W))
}

// class FtqEntry(implicit p: Parameters) extends RVCOREBundle with HasBPUConst {
//   val startAddr = UInt(VAddrBits.W)
...
class FtqRead[T <: Data](private val gen: T)(implicit p: Parameters) extends RVCOREBundle {
...
class FtqToBpuIO(implicit p: Parameters) extends RVCOREBundle {
  val redirect = Valid(new BranchPredictionRedirect)
  val update = Valid(new BranchPredictionUpdate)
  val enq_ptr = Output(new FtqPtr)
}

class FtqToIfuIO(implicit p: Parameters) extends RVCOREBundle with HasCircularQueuePtrHelper {
...
trait HasBackendRedirectInfo extends HasRVCOREParameter {
  def numRedirectPcRead = exuParameters.JmpCnt + exuParameters.AluCnt + 1
  def isLoadReplay(r: Valid[Redirect]) = r.bits.flushItself()
}

class FtqToCtrlIO(implicit p: Parameters) extends RVCOREBundle with HasBackendRedirectInfo {
...
class FTBEntryGen(implicit p: Parameters) extends RVCOREModule with HasBackendRedirectInfo with HasBPUParameter {
  val io = IO(new Bundle {
...
class Ftq(implicit p: Parameters) extends RVCOREModule with HasCircularQueuePtrHelper
  ...
  RVCOREError(isBefore(bpuPtr, ifuPtr) && !isFull(bpuPtr, ifuPtr), "\nifuPtr is before bpuPtr!\n")
  ...
  when (toIfuPcBundle.fallThruError && entry_hit_status(ifuPtr.value) === h_hit) {
    when (io.toIfu.req.fire &&
      ...
      // RVCOREError(true.B, "FTB false hit by fallThroughError, startAddr: %x, fallTHru: %x\n", io.toIfu.req.bits.startAddr, io.toIfu.req.bits.nextStartAddr)
    }
    RVCOREDebug(true.B, "fallThruError! start:%x, fallThru:%x\n", io.toIfu.req.bits.startAddr, io.toIfu.req.bits.nextStartAddr)
  }

  RVCOREPerfAccumulate(f"fall_through_error_to_ifu", toIfuPcBundle.fallThruError && entry_hit_status(ifuPtr.value) === h_hit &&
  ...
  when (RegNext(hit_pd_valid)) {
    ...
    RVCOREDebug(has_false_hit, "FTB false hit by br or jal or hit_pd, startAddr: %x\n", pdWb.bits.pc(0))
  ...
  class RedirectGen(implicit p: Parameters) extends RVCOREModule
    ...
    RVCOREError((io.debug_diff.valid || realRedirect.valid) && io.debug_diff.asUInt =/= io.out.asUInt, "redirect wrong")
  ...
  switch (bpu_ftb_update_stall) {
    ...
    is (3.U) {
      RVCOREError(true.B, "bpu_ftb_update_stall should be 0, 1 or 2")
    }
  }
  ...
  if(cacheParams.hasPrefetch){
    ...
    RVCOREError(isBefore(bpuPtr, prefetchPtr) && !isFull(bpuPtr, prefetchPtr), "\nprefetchPtr is before bpuPtr!\n")
    RVCOREError(isBefore(prefetchPtr, ifuPtr) && !isFull(ifuPtr, prefetchPtr), "\nifuPtr is before prefetchPtr!\n")
  }
  else {
  ...
  for (i <- 0 until PredictWidth) {
    ...
    RVCOREDebug(v && do_commit && isCfi, p"cfi_update: isBr(${isBr}) pc(${Hexadecimal(pc)}) " +
  ...
  RVCOREPerfAccumulate("entry", validEntries)
  RVCOREPerfAccumulate("bpu_to_ftq_stall", enq.valid && !enq.ready)
  RVCOREPerfAccumulate("mispredictRedirect", perf_redirect.valid && RedirectLevel.flushAfter === perf_redirect.bits.level)
  RVCOREPerfAccumulate("replayRedirect", perf_redirect.valid && RedirectLevel.flushItself(perf_redirect.bits.level))
  RVCOREPerfAccumulate("predecodeRedirect", fromIfuRedirect.valid)

  RVCOREPerfAccumulate("to_ifu_bubble", io.toIfu.req.ready && !io.toIfu.req.valid)

  RVCOREPerfAccumulate("to_ifu_stall", io.toIfu.req.valid && !io.toIfu.req.ready)
  RVCOREPerfAccumulate("from_bpu_real_bubble", !enq.valid && enq.ready && allowBpuIn)
  RVCOREPerfAccumulate("bpu_to_ifu_bubble", bpuPtr === ifuPtr)
  ...
  for((key, value) <- perfCountsMap) {
    RVCOREPerfAccumulate(key, value)
  }

  // --------------------------- Debug --------------------------------
  // RVCOREDebug(enq_fire, p"enq! " + io.fromBpu.resp.bits.toPrintable)
  RVCOREDebug(io.toIfu.req.fire, p"fire to ifu " + io.toIfu.req.bits.toPrintable)
  RVCOREDebug(do_commit, p"deq! [ptr] $do_commit_ptr\n")
  RVCOREDebug(true.B, p"[bpuPtr] $bpuPtr, [ifuPtr] $ifuPtr, [ifuWbPtr] $ifuWbPtr [commPtr] $commPtr\n")
  RVCOREDebug(true.B, p"[in] v:${io.fromBpu.resp.valid} r:${io.fromBpu.resp.ready} " +
    p"[out] v:${io.toIfu.req.valid} r:${io.toIfu.req.ready}\n")
  RVCOREDebug(do_commit, p"[deq info] cfiIndex: $commit_cfi, $commit_pc_bundle, target: ${Hexadecimal(commit_target)}\n")
