...
trait ITTageParams extends HasRVCOREParameter with HasBPUParameter {
...
abstract class ITTageBundle(implicit p: Parameters)
  extends RVCOREBundle with ITTageParams with BPUUtils

abstract class ITTageModule(implicit p: Parameters)
  extends RVCOREModule with ITTageParams with BPUUtils
{}
...
class ITTageMeta(implicit p: Parameters) extends RVCOREBundle with ITTageParams{
...
(
  ...
  RVCOREPerfAccumulate(f"ittage_table_bank_conflict", bank_conflict)
  ...
  RVCOREPerfAccumulate("ittage_table_updates", io.update.valid)
  RVCOREPerfAccumulate("ittage_table_hits", io.resp.valid)
  ...
  if (BPUDebug && debug) {
    ...
    RVCOREDebug(io.update.valid,
      ...
      p" in idx $update_idx\n")
    RVCOREDebug(RegNext(io.req.fire) && !s1_req_rhit, "TageTableResp: no hits!\n")


    // ------------------------------Debug-------------------------------------
    val valids = RegInit(0.U.asTypeOf(Vec(nRows, Bool())))
    when (io.update.valid) { valids(update_idx) := true.B }
    RVCOREDebug("ITTAGE Table usage:------------------------\n")
...
class ITTage(implicit p: Parameters) extends BaseITTage {
  ...
  RVCOREDebug(io.s2_fire, p"hit_taken_jalr:")
  ...
  when (updateValid) {
    when (updateMeta.provider.valid) {
      ...
      RVCOREDebug(true.B, p"update provider $provider, pred cycle ${updateMeta.pred_cycle.getOrElse(0.U)}\n")
      ...
      when (usedAltpred && updateMisPred) { // update altpred if used as pred
        RVCOREDebug(true.B, p"update altprovider $altProvider, pred cycle ${updateMeta.pred_cycle.getOrElse(0.U)}\n")
  ...
  when (updateValid && updateMisPred && !(providerCorrect && providerUnconf)) {
    ...
    when (allocate.valid) {
      RVCOREDebug(true.B, p"allocate new table entry, pred cycle ${updateMeta.pred_cycle.getOrElse(0.U)}\n")
  ...
  RVCOREPerfAccumulate(s"ittage_reset_u", updateResetU)
  ...
  RVCOREPerfAccumulate(f"ittage_write_blocks_read", !io.s1_ready)
  // Debug and perf info

  def pred_perf(name: String, cond: Bool)   = RVCOREPerfAccumulate(s"${name}_at_pred", cond && io.s2_fire)
  def commit_perf(name: String, cond: Bool) = RVCOREPerfAccumulate(s"${name}_at_commit", cond && updateValid)
  ...
  // RVCOREPerfAccumulate("ittage_provider_right")
  RVCOREPerfAccumulate("updated", updateValid)

  if (debug) {
    // for (b <- 0 until ITTageBanks) {
    //   val m = updateMetas(b)
    //   // val bri = u.metas(b)
    //   RVCOREDebug(updateValids(b), "update(%d): pc=%x, cycle=%d, hist=%x, taken:%b, misPred:%d, bimctr:%d, pvdr(%d):%d, altDiff:%d, pvdrU:%d, pvdrCtr:%d, alloc(%d):%d\n",
    ...
    RVCOREDebug("s2_fireOnLastCycle: resp: pc=%x, target=%x, hit=%b, taken=%b\n",
      debug_pc_s2, io.out.resp.s2.getTarget, s2_provided, s2_tageTaken)
    for (i <- 0 until ITTageNTables) {
      RVCOREDebug("TageTable(%d): valids:%b, resp_ctrs:%b, resp_us:%b, target:%x\n",
        i.U, VecInit(s2_resps(i).valid).asUInt, s2_resps(i).bits.ctr,
        s2_resps(i).bits.u, s2_resps(i).bits.target)
    }
  }
  RVCOREDebug(updateValid, p"pc: ${Hexadecimal(update.pc)}, target: ${Hexadecimal(update.full_target)}\n")
  RVCOREDebug(updateValid, updateMeta.toPrintable+p"\n")
  RVCOREDebug(updateValid, p"correct(${!updateMisPred})\n")
