...
import rvcore.cache.mmu.CAMTemplate

trait MicroBTBParams extends HasRVCOREParameter with HasBPUParameter {
...
class NewMicroBTBEntry(implicit p: Parameters) extends RVCOREBundle with MicroBTBParams {
  // val valid = Bool()
  ...
  with MicroBTBParams with HasPerfEvents
{
  ...
  class MicroBTBOutMeta extends RVCOREBundle {
    val ftPred = UInt(ftPredBits.W)
  }

  class FallThruPred extends RVCOREModule with MicroBTBParams {
    ...
    RVCOREPerfAccumulate("num_decays", doing_decay)
    RVCOREPerfAccumulate("num_writes", io.wen)
  ...
  RVCOREDebug(p"uBTB entry, read_pc=${Hexadecimal(s0_pc)}\n")
  ...
  // RVCOREDebug("req_v=%b, req_pc=%x, hit=%b\n", io.s1_fire, s1_pc, bank.read_hit)
  RVCOREDebug("target=%x\n", io.out.resp.s1.getTarget)

  RVCOREDebug(u_valid, "[update]Update from ftq\n")
  RVCOREDebug(u_valid, "[update]update_pc=%x, tag=%x\n", u_pc, ubtbAddr.getTag(u_pc))
  RVCOREDebug(u_valid, "[update]taken_mask=%b, brValids=%b, jmpValid=%b\n",
    u_br_taken_mask.asUInt, update.ftb_entry.brValids.asUInt, update.ftb_entry.jmpValid)

  // RVCOREPerfAccumulate("ubtb_read_hits", RegNext(io.s1_fire) && bank.read_hit)
  // RVCOREPerfAccumulate("ubtb_read_misses", RegNext(io.s1_fire) && !bank.read_hit)

  // RVCOREPerfAccumulate("ubtb_commit_hits", u_valid && u_meta.hit)
  // RVCOREPerfAccumulate("ubtb_commit_misses", u_valid && !u_meta.hit)

  RVCOREPerfAccumulate("ubtb_update_on_mispred", u_valid && RegNext(update_mispred))
  RVCOREPerfAccumulate("ubtb_update_on_redirected_by_s2", u_valid && RegNext(update_redirected))
  RVCOREPerfAccumulate("ubtb_update_eliminated", io.update.valid && !(update_mispred || update_redirected))

  RVCOREPerfAccumulate("ubtb_resp_invalid_by_update", io.s1_fire && pred_may_invalid_by_update && shouldNotFallThru)
  RVCOREPerfAccumulate("ubtb_resp_invalid_by_ftpred", io.s1_fire && !pred_may_invalid_by_update && !shouldNotFallThru)

  RVCOREPerfAccumulate("ubtb_update_ft_mispred", RegNext(io.update.valid) && u_ftMisPred)
  RVCOREPerfAccumulate("ubtb_update_ft_pred_correct", RegNext(io.update.valid) && !u_ftMisPred)
