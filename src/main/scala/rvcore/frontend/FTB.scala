...
trait FTBParams extends HasRVCOREParameter with HasBPUConst {
...
class FtbSlot(val offsetLen: Int, val subOffsetLen: Option[Int] = None)(implicit p: Parameters) extends RVCOREBundle with FTBParams {
...
class FTBEntry(implicit p: Parameters) extends RVCOREBundle with FTBParams with BPUUtils {
  ...
  def display(cond: Bool): Unit = {
    RVCOREDebug(cond, p"-----------FTB entry----------- \n")
    RVCOREDebug(cond, p"v=${valid}\n")
    for(i <- 0 until numBr) {
      RVCOREDebug(cond, p"[br$i]: v=${allSlotsForBr(i).valid}, offset=${allSlotsForBr(i).offset}," +
        p"lower=${Hexadecimal(allSlotsForBr(i).lower)}\n")
    }
    RVCOREDebug(cond, p"[tailSlot]: v=${tailSlot.valid}, offset=${tailSlot.offset}," +
      p"lower=${Hexadecimal(tailSlot.lower)}, sharing=${tailSlot.sharing}}\n")
    RVCOREDebug(cond, p"pftAddr=${Hexadecimal(pftAddr)}, carry=$carry\n")
    RVCOREDebug(cond, p"isCall=$isCall, isRet=$isRet, isjalr=$isJalr\n")
    RVCOREDebug(cond, p"last_may_be_rvi_call=$last_may_be_rvi_call\n")
    RVCOREDebug(cond, p"------------------------------- \n")
  }

}

class FTBEntryWithTag(implicit p: Parameters) extends RVCOREBundle with FTBParams with BPUUtils {
  val entry = new FTBEntry
  val tag = UInt(tagSize.W)
  def display(cond: Bool): Unit = {
    entry.display(cond)
    RVCOREDebug(cond, p"tag is ${Hexadecimal(tag)}\n------------------------------- \n")
  }
}

class FTBMeta(implicit p: Parameters) extends RVCOREBundle with FTBParams {
...
// class UpdateQueueEntry(implicit p: Parameters) extends RVCOREBundle with FTBParams {
...
class FTB(implicit p: Parameters) extends BasePredictor with FTBParams with BPUUtils
  ...
  class FTBBank(val numSets: Int, val nWays: Int) extends RVCOREModule with BPUUtils {
    ...
    for (n <- 1 to numWays) {
      RVCOREPerfAccumulate(f"ftb_pred_${n}_way_hit", PopCount(total_hits) === n.U)
      RVCOREPerfAccumulate(f"ftb_update_${n}_way_hit", PopCount(u_total_hits) === n.U)
    }
    ...
    // RVCOREDebug(!hit, "FTB not hit, alloc a way: %d\n", allocWriteWay)
    ...
    for (i <- 0 until numWays) {
      RVCOREPerfAccumulate(f"ftb_replace_way$i", u_valid && io.update_write_alloc && OHToUInt(u_mask) === i.U)
      RVCOREPerfAccumulate(f"ftb_replace_way${i}_has_empty", u_valid && io.update_write_alloc && !ftb_r_entries.map(_.valid).reduce(_&&_) && OHToUInt(u_mask) === i.U)
      RVCOREPerfAccumulate(f"ftb_hit_way$i", hit && !io.update_access && hit_way === i.U)
    }
  ...
  RVCOREDebug("req_v=%b, req_pc=%x, ready=%b (resp at next cycle)\n", io.s0_fire, s0_pc, ftbBank.io.req_pc.ready)
  RVCOREDebug("s2_hit=%b, hit_way=%b\n", s2_hit, writeWay.asUInt)
  RVCOREDebug("s2_br_taken_mask=%b, s2_real_taken_mask=%b\n",
    io.in.bits.resp_in(0).s2.full_pred.br_taken_mask.asUInt, io.out.resp.s2.full_pred.real_slot_taken_mask().asUInt)
  RVCOREDebug("s2_target=%x\n", io.out.resp.s2.getTarget)

  ftb_entry.display(true.B)

  RVCOREPerfAccumulate("ftb_read_hits", RegNext(io.s0_fire) && s1_hit)
  RVCOREPerfAccumulate("ftb_read_misses", RegNext(io.s0_fire) && !s1_hit)

  RVCOREPerfAccumulate("ftb_commit_hits", RegNext(io.update.valid) && u_meta.hit)
  RVCOREPerfAccumulate("ftb_commit_misses", RegNext(io.update.valid) && !u_meta.hit)

  RVCOREPerfAccumulate("ftb_update_req", io.update.valid)
  RVCOREPerfAccumulate("ftb_update_ignored", io.update.valid && io.update.bits.old_entry)
  RVCOREPerfAccumulate("ftb_updated", u_valid)
