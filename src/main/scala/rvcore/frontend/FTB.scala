...
class FTB(implicit p: Parameters) extends BasePredictor with FTBParams with BPUUtils
  ...
  class FTBBank(val numSets: Int, val nWays: Int) extends RVCOREModule with BPUUtils {
    ...
    for (i <- 0 until numWays) {
      RVCOREPerfAccumulate(f"ftb_replace_way$i", u_valid && io.update_write_alloc && u_way === i.U)
      RVCOREPerfAccumulate(f"ftb_replace_way${i}_has_empty", u_valid && io.update_write_alloc && !ftb_r_entries.map(_.valid).reduce(_&&_) && u_way === i.U)
  ...
  RVCOREPerfAccumulate("ftb_commit_hits", io.update.valid && u_meta.hit)
  RVCOREPerfAccumulate("ftb_commit_misses", io.update.valid && !u_meta.hit)
