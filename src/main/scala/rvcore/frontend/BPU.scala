...
trait HasBPUConst extends HasRVCOREParameter {
...
trait HasBPUParameter extends HasRVCOREParameter with HasBPUConst {
...
class BPUCtrl(implicit p: Parameters) extends RVCOREBundle {
...
trait BPUUtils extends HasRVCOREParameter {
  // circular shifting
...
// class BranchPredictionUpdate(implicit p: Parameters) extends RVCOREBundle with HasBPUConst {
...
class BasePredictorInput (implicit p: Parameters) extends RVCOREBundle with HasBPUConst {
  def nInputs = 1
...
class BasePredictorOutput (implicit p: Parameters) extends RVCOREBundle with HasBPUConst {
...
class BasePredictorIO (implicit p: Parameters) extends RVCOREBundle with HasBPUConst {
...
abstract class BasePredictor(implicit p: Parameters) extends RVCOREModule 
...
class BpuToFtqIO(implicit p: Parameters) extends RVCOREBundle {
  val resp = DecoupledIO(new BpuToFtqBundle())
}

class PredictorIO(implicit p: Parameters) extends RVCOREBundle {
...
@chiselName
class Predictor(implicit p: Parameters) extends RVCOREModule with HasBPUConst with HasPerfEvents with HasCircularQueuePtrHelper {
  ...
  RVCOREError(!resp.s1.is_minimal, "s1 should be minimal!\n")
  ...
  RVCOREError(resp.s2.is_minimal, "s2 should not be minimal!\n")
  ...
  RVCOREPerfAccumulate("s2_redirect_because_target_diff", s2_fire && s2_redirect_s1_last_pred_vec(0))
  RVCOREPerfAccumulate("s2_redirect_because_branch_num_diff", s2_fire && s2_redirect_s1_last_pred_vec(1))
  RVCOREPerfAccumulate("s2_redirect_because_direction_diff", s2_fire && s2_redirect_s1_last_pred_vec(2))
  RVCOREPerfAccumulate("s2_redirect_because_cfi_idx_diff", s2_fire && s2_redirect_s1_last_pred_vec(3))
  // RVCOREPerfAccumulate("s2_redirect_because_shouldShiftVec_diff", s2_fire && s2_redirect_s1_last_pred_vec(4))
  // RVCOREPerfAccumulate("s2_redirect_because_brTaken_diff", s2_fire && s2_redirect_s1_last_pred_vec(5))
  RVCOREPerfAccumulate("s2_redirect_because_fallThroughError", s2_fire && resp.s2.fallThruError)

  RVCOREPerfAccumulate("s2_redirect_when_taken", s2_redirect && resp.s2.taken && resp.s2.full_pred.hit)
  RVCOREPerfAccumulate("s2_redirect_when_not_taken", s2_redirect && !resp.s2.taken && resp.s2.full_pred.hit)
  RVCOREPerfAccumulate("s2_redirect_when_not_hit", s2_redirect && !resp.s2.full_pred.hit)
  ...
  RVCOREPerfAccumulate(f"s3_redirect_on_br_taken", s3_fire && s3_redirect_on_br_taken)
  RVCOREPerfAccumulate(f"s3_redirect_on_jalr_target", s3_fire && s3_redirect_on_jalr_target)
  RVCOREPerfAccumulate(f"s3_redirect_on_others", s3_redirect && !(s3_redirect_on_br_taken || s3_redirect_on_jalr_target))
  ...
  RVCOREError(isBefore(redirect.cfiUpdate.histPtr, s3_ghist_ptr) && do_redirect.valid, p"s3_ghist_ptr ${s3_ghist_ptr} exceeds redirect histPtr ${redirect.cfiUpdate.histPtr}\n")
  RVCOREError(isBefore(redirect.cfiUpdate.histPtr, s2_ghist_ptr) && do_redirect.valid, p"s2_ghist_ptr ${s2_ghist_ptr} exceeds redirect histPtr ${redirect.cfiUpdate.histPtr}\n")
  RVCOREError(isBefore(redirect.cfiUpdate.histPtr, s1_ghist_ptr) && do_redirect.valid, p"s1_ghist_ptr ${s1_ghist_ptr} exceeds redirect histPtr ${redirect.cfiUpdate.histPtr}\n")

  RVCOREDebug(RegNext(reset.asBool) && !reset.asBool, "Reseting...\n")
  RVCOREDebug(io.ftq_to_bpu.update.valid, p"Update from ftq\n")
  RVCOREDebug(io.ftq_to_bpu.redirect.valid, p"Redirect from ftq\n")

  RVCOREDebug("[BP0]                 fire=%d                      pc=%x\n", s0_fire, s0_pc)
  RVCOREDebug("[BP1] v=%d r=%d cr=%d fire=%d             flush=%d pc=%x\n",
    s1_valid, s1_ready, s1_components_ready, s1_fire, s1_flush, s1_pc)
  RVCOREDebug("[BP2] v=%d r=%d cr=%d fire=%d redirect=%d flush=%d pc=%x\n",
  s2_valid, s2_ready, s2_components_ready, s2_fire, s2_redirect, s2_flush, s2_pc)
  RVCOREDebug("[BP3] v=%d r=%d cr=%d fire=%d redirect=%d flush=%d pc=%x\n",
  s3_valid, s3_ready, s3_components_ready, s3_fire, s3_redirect, s3_flush, s3_pc)
  RVCOREDebug("[FTQ] ready=%d\n", io.bpu_to_ftq.resp.ready)
  RVCOREDebug("resp.s1.target=%x\n", resp.s1.getTarget)
  RVCOREDebug("resp.s2.target=%x\n", resp.s2.getTarget)
  // RVCOREDebug("s0_ghist: %b\n", s0_ghist.predHist)
  // RVCOREDebug("s1_ghist: %b\n", s1_ghist.predHist)
  // RVCOREDebug("s2_ghist: %b\n", s2_ghist.predHist)
  // RVCOREDebug("s2_predicted_ghist: %b\n", s2_predicted_ghist.predHist)
  RVCOREDebug(p"s0_ghist_ptr: $s0_ghist_ptr\n")
  RVCOREDebug(p"s1_ghist_ptr: $s1_ghist_ptr\n")
  RVCOREDebug(p"s2_ghist_ptr: $s2_ghist_ptr\n")
  RVCOREDebug(p"s3_ghist_ptr: $s3_ghist_ptr\n")
  ...
  RVCOREPerfAccumulate("s2_redirect", s2_redirect)
  RVCOREPerfAccumulate("s3_redirect", s3_redirect)
  RVCOREPerfAccumulate("s1_not_valid", !s1_valid)
