...
trait TageParams extends HasBPUConst with HasRVCOREParameter {
...
abstract class TageBundle(implicit p: Parameters)
  extends RVCOREBundle with TageParams with BPUUtils

abstract class TageModule(implicit p: Parameters)
  extends RVCOREModule with TageParams with BPUUtils
  {}
...
trait TBTParams extends HasRVCOREParameter with TageParams {
  val BtSize = 2048
  val bypassEntries = 8
}

@chiselName
class TageBTable(implicit p: Parameters) extends RVCOREModule with TBTParams{
...
(
  ...
  val SRAM_SIZE = 32 // physical size
  ...
  if (EnableGHistDiff) {
    ...
    RVCOREError(idx_history =/= idx_fh.folded_hist, p"tage table $tableIdx has different fh," +
  ...
  if (EnableGHistDiff) {
    ...
    RVCOREError(update_idx_history =/= update_idx_fh.folded_hist && io.update.mask.reduce(_||_),
  ...
  RVCOREPerfAccumulate(f"tage_table_bank_conflict", bank_conflict)
  ...
  for (i <- 0 until numBr) {
    for (b <- 0 until nBanks) {
      ...
      RVCOREPerfAccumulate(f"tage_table_bank_${b}_wrbypass_enq_$i", io.update.mask(i) && update_req_bank_1h(b) && !wrbypass.io.hit)
      RVCOREPerfAccumulate(f"tage_table_bank_${b}_wrbypass_hit_$i", io.update.mask(i) && update_req_bank_1h(b) &&  wrbypass.io.hit)
    }
  }

  for (b <- 0 until nBanks) {
    val not_silent_update = per_bank_not_silent_update(b)
    RVCOREPerfAccumulate(f"tage_table_bank_${b}_real_updates",
      io.update.mask.reduce(_||_) && update_req_bank_1h(b) && not_silent_update.reduce(_||_))
    RVCOREPerfAccumulate(f"tage_table_bank_${b}_silent_updates_eliminated",
      io.update.mask.reduce(_||_) && update_req_bank_1h(b) && !not_silent_update.reduce(_||_))
  }

  RVCOREPerfAccumulate("tage_table_hits", PopCount(io.resps.map(_.valid)))
  
  for (b <- 0 until nBanks) {
    RVCOREPerfAccumulate(f"tage_table_bank_${b}_update_req", io.update.mask.reduce(_||_) && update_req_bank_1h(b))
    for (i <- 0 until numBr) {
      val li = i
      val pidx = get_phy_br_idx(update_unhashed_idx, li)
      RVCOREPerfAccumulate(f"tage_table_bank_${b}_br_li_${li}_updated", table_banks(b).io.w.req.valid && table_banks(b).io.w.req.bits.waymask.get(pidx))
      val pi = i
      RVCOREPerfAccumulate(f"tage_table_bank_${b}_br_pi_${pi}_updated", table_banks(b).io.w.req.valid && table_banks(b).io.w.req.bits.waymask.get(pi))
    }
  }
  ...
  for (i <- 0 until numBr) {
    ...
    RVCOREDebug(RegNext(io.req.fire) && !req_rhits(i), p"TageTableResp_$i: not hit!\n")
  }
  ...
  RVCOREDebug("Table usage:------------------------\n")
  RVCOREDebug("%d out of %d rows are valid\n", PopCount(valids), nRows.U)
...
class Tage(implicit p: Parameters) extends BaseTage {
  ...
  class TageTableInfo(implicit p: Parameters) extends RVCOREBundle {
  ...
  for (i <- 0 until numBr) {
    ...
    RVCOREPerfAccumulate(f"tage_bank_${i}_use_alt_pred", hasUpdate && updateUseAlt)
    RVCOREPerfAccumulate(f"tage_bank_${i}_alt_correct", hasUpdate && updateUseAlt && updateAltCorrect)
    RVCOREPerfAccumulate(f"tage_bank_${i}_alt_wrong", hasUpdate && updateUseAlt && !updateAltCorrect)
    RVCOREPerfAccumulate(f"tage_bank_${i}_alt_differs", hasUpdate && updateAltDiffers)
    RVCOREPerfAccumulate(f"tage_bank_${i}_use_alt_on_na_ctr_updated", hasUpdate && updateAltDiffers && updateProvided && updateProviderWeak)
    RVCOREPerfAccumulate(f"tage_bank_${i}_use_alt_on_na_ctr_inc", hasUpdate && updateAltDiffers && updateProvided && updateProviderWeak &&  updateAltCorrect)
    RVCOREPerfAccumulate(f"tage_bank_${i}_use_alt_on_na_ctr_dec", hasUpdate && updateAltDiffers && updateProvided && updateProviderWeak && !updateAltCorrect)
    
    RVCOREPerfAccumulate(f"tage_bank_${i}_na", hasUpdate && updateProvided && updateProviderWeak)
    RVCOREPerfAccumulate(f"tage_bank_${i}_use_na_correct", hasUpdate && updateProvided && updateProviderWeak && !updateUseAlt && !updateMispred)
    RVCOREPerfAccumulate(f"tage_bank_${i}_use_na_wrong",   hasUpdate && updateProvided && updateProviderWeak && !updateUseAlt &&  updateMispred)

    updateMeta.use_alt_on_na.map(uaon => RVCOREPerfAccumulate(f"tage_bank_${i}_use_alt_on_na", hasUpdate && uaon(i)))
    ...
    RVCOREPerfAccumulate(f"tage_bank_${i}_update_allocate_failure", needToAllocate && !canAllocate)
    RVCOREPerfAccumulate(f"tage_bank_${i}_update_allocate_success", needToAllocate &&  canAllocate)
    RVCOREPerfAccumulate(s"tage_bank_${i}_mispred", hasUpdate && updateMispred)
    RVCOREPerfAccumulate(s"tage_bank_${i}_reset_u", updateResetU(i))
    for (t <- 0 to TageNTables) {
      RVCOREPerfAccumulate(f"tage_bank_${i}_tick_inc_${t}", needToAllocate && tickInc && tickIncVal === t.U)
      RVCOREPerfAccumulate(f"tage_bank_${i}_tick_dec_${t}", needToAllocate && tickDec && tickDecVal === t.U)
    }
  }
  ...
  RVCOREPerfAccumulate(f"tage_write_blocks_read", !io.s1_ready)

  def pred_perf(name: String, cnt: UInt)   = RVCOREPerfAccumulate(s"${name}_at_pred", cnt)
  def commit_perf(name: String, cnt: UInt) = RVCOREPerfAccumulate(s"${name}_at_commit", cnt)
  ...
  for (b <- 0 until TageBanks) {
    val m = updateMeta
    // val bri = u.metas(b)
    RVCOREDebug(updateValids(b), "update(%d): pc=%x, cycle=%d, taken:%b, misPred:%d, bimctr:%d, pvdr(%d):%d, altDiff:%d, pvdrU:%d, pvdrCtr:%d, alloc:%b\n",
  ...
  RVCOREDebug("req: v=%d, pc=0x%x\n", io.s0_fire, s0_pc)
  RVCOREDebug("s1_fire:%d, resp: pc=%x\n", io.s1_fire, debug_pc_s1)
  RVCOREDebug("s2_fireOnLastCycle: resp: pc=%x, target=%x, hits=%b, takens=%b\n",
    debug_pc_s2, io.out.resp.s2.getTarget, s2_provideds.asUInt, s2_tageTakens.asUInt)

  for (b <- 0 until TageBanks) {
    for (i <- 0 until TageNTables) {
      RVCOREDebug("bank(%d)_tage_table(%d): valid:%b, resp_ctr:%d, resp_us:%d\n",
        b.U, i.U, s2_resps(i)(b).valid, s2_resps(i)(b).bits.ctr, s2_resps(i)(b).bits.u)
    }
  }
    // RVCOREDebug(io.update.valid && updateIsBr, p"update: sc: ${updateSCMeta}\n")
    // RVCOREDebug(true.B, p"scThres: use(${useThreshold}), update(${updateThreshold})\n")
}
