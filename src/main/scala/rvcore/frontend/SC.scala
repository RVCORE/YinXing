...
class SCMeta(val ntables: Int)(implicit p: Parameters) extends RVCOREBundle with HasSCParameter {
...
class SCTable(val nRows: Int, val ctrBits: Int, val histLen: Int)(implicit p: Parameters)
  ...
  val u = io.update
  RVCOREDebug(io.req.valid,
    p"scTableReq: pc=0x${Hexadecimal(io.req.bits.pc)}, " +
    p"s0_idx=${s0_idx}\n")
  RVCOREDebug(RegNext(io.req.valid),
    p"scTableResp: s1_idx=${s1_idx}," +
    p"ctr:${io.resp.ctrs}\n")
  RVCOREDebug(io.update.mask.reduce(_||_),
...
class SCThreshold(val ctrBits: Int = 6)(implicit p: Parameters) extends SCBundle {
  ...
  def update(cause: Bool): SCThreshold = {
    ...
    // RVCOREDebug(true.B, p"scThres Update: cause${cause} newCtr ${newCtr} newThres ${newThres}\n")
    res
  }
}
  ...
  if (EnableSC) {
    ...
    for (w <- 0 until TageBanks) {
      ...
      when (s2_provideds(w)) {
        ...
        RVCOREDebug(p"---------tage_bank_${w} provided so that sc used---------\n")
        when (s2_sumAboveThresholds(s2_chooseBit)) {
          ...
          RVCOREDebug(p"pc(${Hexadecimal(debug_pc)}) SC(${w.U}) overriden pred to ${pred}\n")
        }
      }
      ...
      when (updateValids(w) && updateSCMeta.scUsed(w)) {
        ...
        when (scPred =/= tagePred && sumAbs >= thres - 4.U && sumAbs <= thres - 2.U) {
          ...
          RVCOREDebug(p"scThres $w update: old ${useThresholds(w)} --> new ${newThres.thres}\n")
        }
        ...
        when (scPred =/= taken || !sumAboveThreshold) {
          ...
          RVCOREDebug(p"bank(${w}), update: sc: ${updateSCMeta}\n")
    ...
    RVCOREPerfAccumulate("sc_update_on_mispred", PopCount(update_on_mispred))
    RVCOREPerfAccumulate("sc_update_on_unconf", PopCount(update_on_unconf))
    RVCOREPerfAccumulate("sc_mispred_but_tage_correct", PopCount(sc_misp_tage_corr))
    RVCOREPerfAccumulate("sc_correct_and_tage_wrong", PopCount(sc_corr_tage_misp))
