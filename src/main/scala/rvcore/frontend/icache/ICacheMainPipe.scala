...
package rvcore.frontend.icache
...
class ICacheMainPipe(implicit p: Parameters) extends ICacheModule
{
  ...
  RVCOREPerfAccumulate("icache_bubble_s0_tlb_miss",    s0_valid && tlb_has_miss )
  ...
  (0 until nWays).map{ w =>
    RVCOREPerfAccumulate("line_0_hit_way_" + Integer.toString(w, 10),  s1_fire && s1_port_hit(0) && OHToUInt(s1_tag_match_vec(0))  === w.U)
  }

  (0 until nWays).map{ w =>
    RVCOREPerfAccumulate("line_0_victim_way_" + Integer.toString(w, 10),  s1_fire && !s1_port_hit(0) && OHToUInt(s1_victim_oh(0))  === w.U)
  }

  (0 until nWays).map{ w =>
    RVCOREPerfAccumulate("line_1_hit_way_" + Integer.toString(w, 10),  s1_fire && s1_double_line && s1_port_hit(1) && OHToUInt(s1_tag_match_vec(1))  === w.U)
  }

  (0 until nWays).map{ w =>
    RVCOREPerfAccumulate("line_1_victim_way_" + Integer.toString(w, 10),  s1_fire && s1_double_line && !s1_port_hit(1) && OHToUInt(s1_victim_oh(1))  === w.U)
  }
  ...
  RVCOREError(s2_parity_error.reduce(_||_) && RegNext(RegNext(s1_fire)), "ICache has parity error in MainPaipe!")
  ...
  RVCOREPerfAccumulate("icache_bubble_s2_miss",    s2_valid && !s2_fetch_finish )
