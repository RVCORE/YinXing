...
abstract class ICacheMissUnitModule(implicit p: Parameters) extends RVCOREModule
  with HasICacheParameters

abstract class ICacheMissUnitBundle(implicit p: Parameters) extends RVCOREBundle
  with HasICacheParameters
  ...
  with MemoryOpConstants
{
  ...
  RVCOREPerfAccumulate("entryReq" + Integer.toString(id, 10), io.req.fire())
...
class ICacheMissUnit(edge: TLEdgeOut)(implicit p: Parameters) extends ICacheMissUnitModule
{
  ...
  val entries = (0 until PortNumber) map { i =>
    ...
    RVCOREPerfAccumulate("entryReq" + Integer.toString(i, 10), entry.io.req.fire())
  ...
  (0 until nWays).map{ w =>
    RVCOREPerfAccumulate("line_0_refill_way_" + Integer.toString(w, 10),  entries(0).io.meta_write.valid && OHToUInt(entries(0).io.meta_write.bits.waymask)  === w.U)
    RVCOREPerfAccumulate("line_1_refill_way_" + Integer.toString(w, 10),  entries(1).io.meta_write.valid && OHToUInt(entries(1).io.meta_write.bits.waymask)  === w.U)
  }
