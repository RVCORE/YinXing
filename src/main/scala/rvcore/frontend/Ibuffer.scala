...
class IbufPtr(implicit p: Parameters) extends CircularQueuePtr[IbufPtr](
  p => p(RVCORECoreParamsKey).IBufSize
){
}

class IBufferIO(implicit p: Parameters) extends RVCOREBundle {
...
class Ibuffer(implicit p: Parameters) extends RVCOREModule with HasCircularQueuePtrHelper with HasPerfEvents {
  val io = IO(new IBufferIO)

  class IBufEntry(implicit p: Parameters) extends RVCOREBundle {
    val inst = UInt(32.W)
...
//    dontTouch(inWire.triggered)
  ...
  // Debug info
  RVCOREDebug(io.flush, "IBuffer Flushed\n")

  when(io.in.fire) {
    RVCOREDebug("Enque:\n")
    RVCOREDebug(p"MASK=${Binary(io.in.bits.valid)}\n")
    for(i <- 0 until PredictWidth){
      RVCOREDebug(p"PC=${Hexadecimal(io.in.bits.pc(i))} ${Hexadecimal(io.in.bits.instrs(i))}\n")
    }
  }

  for (i <- 0 until DecodeWidth) {
    RVCOREDebug(io.out(i).fire(), p"deq: ${Hexadecimal(io.out(i).bits.instr)} PC=${Hexadecimal(io.out(i).bits.pc)} v=${io.out(i).valid} r=${io.out(i).ready} " +
      p"excpVec=${Binary(io.out(i).bits.exceptionVec.asUInt)} crossPageIPF=${io.out(i).bits.crossPageIPFFix}\n")
  }

  RVCOREDebug(p"ValidEntries: ${validEntries}\n")
  RVCOREDebug(p"EnqNum: ${numEnq}\n")
  RVCOREDebug(p"DeqNum: ${numDeq}\n")
  ...
  RVCOREPerfAccumulate("flush", io.flush)
  RVCOREPerfAccumulate("hungry", instrHungry)
