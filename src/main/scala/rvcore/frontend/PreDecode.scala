...
import rvcore.backend.decode.isa.predecode.PreDecodeInst

trait HasPdConst extends HasRVCOREParameter with HasICacheParameters with HasIFUConst{
...
class PreDecodeResp(implicit p: Parameters) extends RVCOREBundle with HasPdConst {
...
class PreDecode(implicit p: Parameters) extends RVCOREModule with HasPdConst{
...
class RVCExpander(implicit p: Parameters) extends RVCOREModule {
  val io = IO(new Bundle {
...
class PredCheckerResp(implicit p: Parameters) extends RVCOREBundle with HasPdConst {
...
class PredChecker(implicit p: Parameters) extends RVCOREModule with HasPdConst {
  val io = IO( new Bundle{
...
class FrontendTrigger(implicit p: Parameters) extends RVCOREModule {
  val io = IO(new Bundle(){
  ...
  RVCOREDebug(triggerEnable.asUInt.orR, "Debug Mode: At least one frontend trigger is enabled\n")
  ...
  for (i <- 0 until PredictWidth) {
    ...
    RVCOREDebug(io.triggered(i).getHitFrontend, p"Debug Mode: Predecode Inst No. ${i} has trigger hit vec ${io.triggered(i).frontendHit}" +
