...
import rvcore._
import rvcore.cache.mmu._
import rvcore.frontend.icache._
import utils._
import rvcore.backend.fu.{PMPReqBundle, PMPRespBundle}

trait HasInstrMMIOConst extends HasRVCOREParameter with HasIFUConst{
  def mmioBusWidth = 64
  def mmioBusBytes = mmioBusWidth / 8
  def maxInstrLen = 32
}

trait HasIFUConst extends HasRVCOREParameter{
...
class IfuToFtqIO(implicit p:Parameters) extends RVCOREBundle {
  val pdWb = Valid(new PredecodeWritebackBundle)
}

class FtqInterface(implicit p: Parameters) extends RVCOREBundle {
  val fromFtq = Flipped(new FtqToIfuIO)
  val toFtq   = new IfuToFtqIO
}

class UncacheInterface(implicit p: Parameters) extends RVCOREBundle {
  val fromUncache = Flipped(DecoupledIO(new InsUncacheResp))
  val toUncache   = DecoupledIO( new InsUncacheReq )
}
class NewIFUIO(implicit p: Parameters) extends RVCOREBundle {
...
class LastHalfInfo(implicit p: Parameters) extends RVCOREBundle {
  val valid = Bool()
  val middlePC = UInt(VAddrBits.W)
  def matchThisBlock(startAddr: UInt) = valid && middlePC === startAddr
}

class IfuToPreDecode(implicit p: Parameters) extends RVCOREBundle {
...
class IfuToPredChecker(implicit p: Parameters) extends RVCOREBundle {
...
class NewIFU(implicit p: Parameters) extends RVCOREModule
  ...
  with HasPerfEvents
{
  ...
  class TlbExept(implicit p: Parameters) extends RVCOREBundle{
    val pageFault = Bool()
  ...
  RVCOREPerfAccumulate("fetch_bubble_ftq_not_valid",   !fromFtq.req.valid && fromFtq.req.ready  )
  RVCOREPerfAccumulate("fetch_bubble_pipe_stall",    f0_valid && toICache(0).ready && toICache(1).ready && !f1_ready )
  RVCOREPerfAccumulate("fetch_bubble_icache_0_busy",   f0_valid && !toICache(0).ready  )
  RVCOREPerfAccumulate("fetch_bubble_icache_1_busy",   f0_valid && !toICache(1).ready  )
  RVCOREPerfAccumulate("fetch_flush_backend_redirect",   backend_redirect  )
  RVCOREPerfAccumulate("fetch_flush_wb_redirect",    wb_redirect  )
  RVCOREPerfAccumulate("fetch_flush_bpu_f1_flush",   from_bpu_f1_flush  )
  RVCOREPerfAccumulate("fetch_flush_bpu_f0_flush",   from_bpu_f0_flush  )
  ...
  RVCOREPerfAccumulate("fetch_bubble_icache_not_resp",   f2_valid && !icacheRespAllValid )
  ...
  RVCOREPerfAccumulate("fetch_bubble_ibuffer_not_ready",   io.toIbuffer.valid && !io.toIbuffer.ready )
  ...
  RVCOREPerfAccumulate("predecode_flush_jalFault",   checkJalFault )
  RVCOREPerfAccumulate("predecode_flush_retFault",   checkRetFault )
  RVCOREPerfAccumulate("predecode_flush_targetFault",   checkTargetFault )
  RVCOREPerfAccumulate("predecode_flush_notCFIFault",   checkNotCFIFault )
  RVCOREPerfAccumulate("predecode_flush_incalidTakenFault",   checkInvalidTaken )

  when(checkRetFault){
    RVCOREDebug("startAddr:%x  nextstartAddr:%x  taken:%d    takenIdx:%d\n",
  ...
  RVCOREPerfAccumulate("ifu_req",   io.toIbuffer.fire() )
  RVCOREPerfAccumulate("ifu_miss",  io.toIbuffer.fire() && !f3_hit )
  RVCOREPerfAccumulate("ifu_req_cacheline_0", f3_req_0  )
  RVCOREPerfAccumulate("ifu_req_cacheline_1", f3_req_1  )
  RVCOREPerfAccumulate("ifu_req_cacheline_0_hit",   f3_hit_0 )
  RVCOREPerfAccumulate("ifu_req_cacheline_1_hit",   f3_hit_1 )
  RVCOREPerfAccumulate("frontendFlush",  wb_redirect )
  RVCOREPerfAccumulate("only_0_hit",      f3_perf_info.only_0_hit   && io.toIbuffer.fire()  )
  RVCOREPerfAccumulate("only_0_miss",     f3_perf_info.only_0_miss  && io.toIbuffer.fire()  )
  RVCOREPerfAccumulate("hit_0_hit_1",     f3_perf_info.hit_0_hit_1  && io.toIbuffer.fire()  )
  RVCOREPerfAccumulate("hit_0_miss_1",    f3_perf_info.hit_0_miss_1  && io.toIbuffer.fire()  )
  RVCOREPerfAccumulate("miss_0_hit_1",    f3_perf_info.miss_0_hit_1   && io.toIbuffer.fire() )
  RVCOREPerfAccumulate("miss_0_miss_1",   f3_perf_info.miss_0_miss_1 && io.toIbuffer.fire() )
  RVCOREPerfAccumulate("hit_0_except_1",   f3_perf_info.hit_0_except_1 && io.toIbuffer.fire() )
