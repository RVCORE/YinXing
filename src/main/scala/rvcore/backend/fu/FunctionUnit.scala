...
import utils.RVCOREPerfAccumulate
import rvcore._
import rvcore.backend.fu.fpu._
...
class FuOutput(val len: Int)(implicit p: Parameters) extends RVCOREBundle {
  val data = UInt(len.W)
  val uop = new MicroOp
}

class FunctionUnitInput(val len: Int)(implicit p: Parameters) extends RVCOREBundle {
  val src = Vec(3, UInt(len.W))
  val uop = new MicroOp
}

class FunctionUnitIO(val len: Int)(implicit p: Parameters) extends RVCOREBundle {
...
abstract class FunctionUnit(len: Int = 64)(implicit p: Parameters) extends RVCOREModule {

  val io = IO(new FunctionUnitIO(len))

  RVCOREPerfAccumulate("in_valid", io.in.valid)
  RVCOREPerfAccumulate("in_fire", io.in.fire)
  RVCOREPerfAccumulate("out_valid", io.out.valid)
  RVCOREPerfAccumulate("out_fire", io.out.fire)
