...
class FMA(implicit p: Parameters) extends FPUSubModule {
  ...
  RVCOREError(isFMA && !add_pipe.io.in.ready, "FMA should not be blocked\n")
  ...
  RVCOREPerfAccumulate("fma_partial_issue_fire", io.in.fire && midResult.waitForAdd)
  RVCOREPerfAccumulate("fma_mid_result_in_fire", io.in.fire && midResult.in.valid)
}
