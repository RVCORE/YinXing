...
import rvcore._
import rvcore.backend.fu.util._
...
class PerfCounterIO(implicit p: Parameters) extends RVCOREBundle {
...
class CSRFileIO(implicit p: Parameters) extends RVCOREBundle {
...
//  val d = Output(Bool())    // Debug
  ...
  RVCOREDebug(dpcPrev =/= dpc, "Debug Mode: dpc is altered! Current is %x, previous is %x\n", dpc, dpcPrev)
  ...
  def WriteTdata1(wdata: UInt) = {
    ...
    RVCOREDebug(src2(11, 0) === Tdata1.U && valid && func =/= CSROpType.jmp, p"Debug Mode: tdata1(${tselectPhy})is written, the actual value is ${wdata}\n")
//    tdata1_new.hit := wdata(20)
  ...
  val marchid = RegInit(UInt(XLEN.W), 25.U) // architecture id for RVCore is 25; see https://github.com/riscv/riscv-isa-manual/blob/master/marchid.md
  ...
  val mstatusWMask = (~ZeroExt((
    ...
    GenMask(31, 23)       | // WPRI
    GenMask(16, 15)       | // RVCORE is read-only
    GenMask(10, 9)        | // WPRI
    GenMask(6)            | // WPRI
    GenMask(2)              // WPRI
  ), 64)).asUInt
  ...
  RVCOREDebug(csrio.customCtrl.trigger_enable.asUInt.orR, p"Debug Mode: At least 1 trigger is enabled," +
  ...
  RVCOREDebug(wen, "csr write: pc %x addr %x rdata %x wdata %x func %x\n", cfIn.pc, addr, rdata, wdata, func)
  RVCOREDebug(wen, "pc %x mstatus %x mideleg %x medeleg %x mode %x\n", cfIn.pc, mstatus, mideleg , medeleg, priviledgeMode)
  ...
  when (valid && isDret) {
    ...
    RVCOREDebug("Debug Mode: Dret executed, returning to %x.", retTarget)
  }
  ...
  RVCOREDebug(io.in.valid && isEbreak, s"Debug Mode: an Ebreak is executed, ebreak cause exception ? ${ebreakCauseException}\n")
  ...
  RVCOREDebug(debugIntr, "Debug Mode: debug interrupt is asserted and valid!")
  ...
  RVCOREDebug(raiseIntr, "interrupt: pc=0x%x, %d\n", dexceptionPC, intrNO)
  ...
  RVCOREDebug(hasSingleStep, "Debug Mode: single step exception\n")
  RVCOREDebug(hasTriggerHit, p"Debug Mode: trigger hit, is frontend? ${Binary(csrio.exception.bits.uop.cf.trigger.frontendHit.asUInt)} " +
  ...
  RVCOREDebug(raiseExceptionIntr, "int/exc: pc %x int (%d):%x exc: (%d):%x\n",
    dexceptionPC, intrNO, intrVec, exceptionNO, raiseExceptionVec.asUInt
  )
  RVCOREDebug(raiseExceptionIntr,
  ...
  when (raiseExceptionIntr) {
    ...
    when (raiseDebugExceptionIntr) {
      when (raiseDebugIntr) {
        ...
        RVCOREDebug(raiseDebugIntr, "Debug Mode: Trap to %x at pc %x\n", debugTrapTarget, dpc)
  ...
  RVCOREDebug(raiseExceptionIntr && delegS, "sepc is written!!! pc:%x\n", cfIn.pc)
...
class PFEvent(implicit p: Parameters) extends RVCOREModule with HasCSRConst  {
