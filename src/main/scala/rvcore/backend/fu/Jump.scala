...
import rvcore.backend.decode.isa._

trait HasRedirectOut { this: RVCOREModule =>
  val redirectOutValid = IO(Output(Bool()))
  val redirectOut = IO(Output(new Redirect))
}

class JumpDataModule(implicit p: Parameters) extends RVCOREModule {
...
class Jump(implicit p: Parameters) extends FUWithRedirect {
  ...
  RVCOREDebug(io.in.valid, "In(%d %d) Out(%d %d) Redirect:(%d %d %d)\n",
    io.in.valid,
    io.in.ready,
    io.out.valid,
    io.out.ready,
