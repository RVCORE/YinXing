...
import utils._
import rvcore._
import rvcore.backend.issue.AgeDetector

class InputBuffer(numEntries: Int)(implicit p: Parameters) extends RVCOREModule {
  ...
  when (io.out.fire) {
    for (i <- 0 until numEntries) {
      when (age.io.out(i)) {
        emptyVec(i) := true.B
        RVCOREError(emptyVec(i), "should not deq an empty entry\n")
      }
    }
  }
  ...
  RVCOREPerfHistogram("num_valid", numValid, true.B, 0, numEntries, 1)
  RVCOREPerfAccumulate("aver_num_valid", numValid)
}
