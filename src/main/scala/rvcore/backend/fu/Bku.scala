...
import utils.{LookupTreeDefault, ParallelMux, ParallelXOR, SignExt, RVCOREDebug, RVCOREError, ZeroExt}
import rvcore._
...
class CountModule(implicit p: Parameters) extends RVCOREModule {
...
class ClmulModule(implicit p: Parameters) extends RVCOREModule {
  val io = IO(new Bundle() {
...
class MiscModule(implicit p: Parameters) extends RVCOREModule {
  val io = IO(new Bundle() {
    val src = Vec(2, Input(UInt(XLEN.W)))
    val func = Input(UInt())
    val regEnable = Input(Bool())
...
class HashModule(implicit p: Parameters) extends RVCOREModule {
  val io = IO(new Bundle() {
    val src = Input(UInt(XLEN.W))
    val func = Input(UInt())
    val regEnable = Input(Bool())
...
class BlockCipherModule(implicit p: Parameters) extends RVCOREModule {
  val io = IO(new Bundle() {
    val src = Vec(2, Input(UInt(XLEN.W)))
    val func = Input(UInt())
    val regEnable = Input(Bool())
...
class CryptoModule(implicit p: Parameters) extends RVCOREModule {
  val io = IO(new Bundle() {
    val src = Vec(2, Input(UInt(XLEN.W)))
    val func = Input(UInt())
    val regEnable = Input(Bool())
