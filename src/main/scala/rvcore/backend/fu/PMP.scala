...
import utils._
import rvcore.cache.mmu.{TlbCmd, TlbExceptionBundle}
...
abstract class PMPRVCOREModule(implicit p: Parameters) extends RVCOREModule with PMPConst
...
@chiselName
class PMP(implicit p: Parameters) extends PMPRVCOREModule with HasRVCOREParameter with PMPMethod with PMAMethod with HasCSRConst {
