...
import utils._
import rvcore._
import rvcore.backend.dispatch.Dispatch2Rs
import rvcore.backend.exu.ExuConfig
import rvcore.backend.fu.fpu.FMAMidResultIO
import rvcore.backend.issue.ReservationStationWrapper
import rvcore.backend.regfile.{Regfile, RfReadPort}
import rvcore.backend.rename.{BusyTable, BusyTableReadIO}
import rvcore.mem.{LsqEnqCtrl, LsqEnqIO, MemWaitUpdateReq, SqPtr}

class DispatchArbiter(func: Seq[MicroOp => Bool])(implicit p: Parameters) extends RVCOREModule {
...
class Scheduler(
  ...
  val hasFpRf: Boolean
)(implicit p: Parameters) extends LazyModule with HasRVCOREParameter with HasExuWbHelper {
...
class SchedulerImp(outer: Scheduler) extends LazyModuleImp(outer) with HasRVCOREParameter with HasPerfEvents {
  ...
  class SchedulerExtraIO extends RVCOREBundle {
    // feedback ports
  ...
  RVCOREPerfAccumulate("allocate_valid", PopCount(allocate.map(_.valid)))
  RVCOREPerfAccumulate("allocate_fire", PopCount(allocate.map(_.fire())))
  RVCOREPerfAccumulate("issue_valid", PopCount(io.issue.map(_.valid)))
  RVCOREPerfAccumulate("issue_fire", PopCount(io.issue.map(_.fire)))
