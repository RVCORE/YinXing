...
class ExuBlockImp(outer: ExuBlock)(implicit p: Parameters) extends LazyModuleImp(outer)
  ...
  for (((cfg, fuOut), fastOut) <- flattenFuConfigs.zip(fuBlock.io.writeback).zip(io.fastUopOut)) {
    if (cfg == FmacExeUnitCfg) {
      ...
      RVCOREError(fuOut.valid && !fuOut.ready, "fastUopOut should not be blocked\n")
  ...
  for ((i, (cfg, wb)) <- exclusiveRfWbIdx.zip(exclusiveFuWb)) {
    ...
    RVCOREError(hasWb && outerWb.bits.uop.robIdx =/= scheWb.bits.uop.robIdx,
      "different instruction between io.rfWriteback and fu.writeback\n")
    RVCOREError(hasWb && outerWb.bits.data =/= scheWb.bits.data,
  ...
  for ((cfg, i) <- allWakeupFromRs) {
    ...
    RVCOREError(wbOut.valid && !wbOut.ready, "fast uop wb should not be blocked\n")
    ...
    RVCOREError((wbOut.valid || fuWbValid) && wbOut.bits.uop.robIdx =/= fuWbRobIdx,
