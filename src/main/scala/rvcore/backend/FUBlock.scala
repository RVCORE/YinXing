...
import rvcore.backend.fu.CSRFileIO
import rvcore.backend.fu.fpu.FMAMidResultIO

class WakeUpBundle(numFast: Int, numSlow: Int)(implicit p: Parameters) extends RVCOREBundle {
...
class FUBlockExtraIO(configs: Seq[(ExuConfig, Int)])(implicit p: Parameters) extends RVCOREBundle {
...
class FUBlock(configs: Seq[(ExuConfig, Int)])(implicit p: Parameters) extends RVCOREModule {
  ...
  for ((iss, i) <- io.issue.zipWithIndex) {
    RVCOREPerfAccumulate(s"issue_count_$i", iss.fire())
  }
  RVCOREPerfHistogram("writeback_count", PopCount(io.writeback.map(_.fire())), true.B, 0, numIn, 1)
