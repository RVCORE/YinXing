...
import utils.{RVCOREPerfAccumulate, RVCOREPerfHistogram}
import rvcore._
import rvcore.backend.HasExuWbHelper

class ExuWbArbiter(n: Int, hasFastUopOut: Boolean, fastVec: Seq[Boolean])(implicit p: Parameters) extends RVCOREModule {
...
class WbArbiterImp(outer: WbArbiter)(implicit p: Parameters) extends LazyModuleImp(outer) {
  ...
  for (i <- 0 until outer.numInPorts) {
    RVCOREPerfAccumulate(s"in_valid_$i", io.in(i).valid)
    RVCOREPerfAccumulate(s"in_fire_$i", io.in(i).fire)
  }
  RVCOREPerfHistogram("in_count", PopCount(io.in.map(_.valid)), true.B, 0, outer.numInPorts, 1)
  RVCOREPerfHistogram("out_count", PopCount(io.out.map(_.valid)), true.B, 0, outer.numInPorts, 1)
}
...
class WbArbiterWrapper(
  ...
  lazy val module = new LazyModuleImp(this)
    with HasRVCOREParameter with HasWritebackSourceImp with HasExuWbHelper {
