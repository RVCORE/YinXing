...
@instantiable
abstract class Exu(cfg: ExuConfig)(implicit p: Parameters) extends RVCOREModule {
  ...
  if (config.readIntRf) {
    RVCOREPerfAccumulate("from_int_fire", io.fromInt.fire())
    RVCOREPerfAccumulate("from_int_valid", io.fromInt.valid)
    io.fromInt.ready := !io.fromInt.valid || inReady(readIntFu)
  }

  if (config.readFpRf) {
    RVCOREPerfAccumulate("from_fp_fire", io.fromFp.fire())
    RVCOREPerfAccumulate("from_fp_valid", io.fromFp.valid)
