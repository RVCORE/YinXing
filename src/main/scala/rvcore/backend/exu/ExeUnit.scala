...
import utils._
import rvcore._
import rvcore.backend.fu.fpu.{FMA, FPUSubModule}
import rvcore.backend.fu.{CSR, FUWithRedirect, Fence, FenceToSbuffer}

class FenceIO(implicit p: Parameters) extends RVCOREBundle {
...
class ExeUnit(config: ExuConfig)(implicit p: Parameters) extends Exu(config) {
  ...
  if (config.readIntRf) {
    val in = io.fromInt
    val out = io.out
    RVCOREDebug(in.valid, p"fromInt(${in.valid} ${in.ready}) toInt(${out.valid} ${out.ready})\n")
    RVCOREDebug(io.redirect.valid, p"Redirect:(${io.redirect.valid}) robIdx:${io.redirect.bits.robIdx}\n")
    RVCOREDebug(in.valid, p"src1:${Hexadecimal(in.bits.src(0))} src2:${Hexadecimal(in.bits.src(1))} " +
      p"func:${Binary(in.bits.uop.ctrl.fuOpType)} pc:${Hexadecimal(in.bits.uop.cf.pc)} robIdx:${in.bits.uop.robIdx}\n")
    RVCOREDebug(out.valid, p"out res:${Hexadecimal(out.bits.data)} robIdx:${out.bits.uop.robIdx}\n")
  }
