...
class StatusEntry(params: RSParams)(implicit p: Parameters) extends RVCOREBundle {
  // states
  val valid = Bool()
  val scheduled = Bool()
  val blocked = Bool()
...
class StatusArray(params: RSParams)(implicit p: Parameters) extends RVCOREModule
  ...
  def wakeupMatch(srcInfo: (UInt, UInt)): (Bool, UInt) = {
    ...
    RVCOREError(PopCount(dataMatchVec) > 1.U, p"matchVec ${Binary(dataMatch)} should be one-hot\n")
  ...
  def deqRespSel(i: Int) : (Bool, Bool, UInt, SqPtr) = {
    ...
    RVCOREError(PopCount(mask) > 1.U, p"feedbackVec ${Binary(mask.asUInt)} should be one-hot\n")
    val deqValid = mask.asUInt.orR
    RVCOREError(deqValid && !statusArray(i).valid, p"should not deq an invalid entry $i\n")
    if (params.hasFeedback) {
      RVCOREError(deqValid && !statusArray(i).scheduled, p"should not deq an un-scheduled entry $i\n")
    }
  ...
  def enqUpdate(i: Int): (Bool, StatusEntry) = {
    ...
    RVCOREError(PopCount(updateVec) > 1.U, "should not update the same entry\n")
  ...
  for (((status, statusNext), i) <- statusArray.zip(statusArrayNext).zipWithIndex) {
    ...
    RVCOREError(updateValid(i) && status.valid, p"should not update a valid entry $i\n")
    ...
    RVCOREError(hasIssued && !status.valid, p"should not issue an invalid entry $i\n")
    ...
    if (params.checkWaitBit) {
      ...
      when (deqNotGranted && deqRespType === RSFeedbackType.dataInvalid) {
        ...
        RVCOREError(status.valid && !isAfter(status.sqIdx, RegNext(RegNext(io.stIssuePtr))),
    ...
    RVCOREError(status.valid && status.credit > 0.U && !status.scheduled,
    ...
    RVCOREDebug(status.valid, p"entry[$i]: $status\n")
  }
  ...
  RVCOREPerfHistogram("valid_entries", validEntries, true.B, 0, params.numEntries, 1)
  for (i <- 0 until params.numSrc) {
    ...
    RVCOREPerfAccumulate(s"wait_for_src_$i", PopCount(srcBlockIssue))
    for (j <- 0 until params.allWakeup) {
      val wakeup_j_i = io.wakeupMatch.map(_(i)(j)).zip(statusArray.map(_.valid)).map(p => p._1 && p._2)
      RVCOREPerfAccumulate(s"wakeup_${j}_$i", PopCount(wakeup_j_i).asUInt)
      val criticalWakeup = srcBlockIssue.zip(wakeup_j_i).map(x => x._1 && x._2)
      RVCOREPerfAccumulate(s"critical_wakeup_${j}_$i", PopCount(criticalWakeup))
      ...
      if (i == 2 && j < 2 * exuParameters.FmacCnt) {
        ...
        RVCOREPerfAccumulate(s"critical_wakeup_from_fma_${j}", Mux(isFMA, PopCount(criticalWakeup), 0.U))
      }
    }
  }
  val canIssueEntries = PopCount(io.canIssue)
  RVCOREPerfHistogram("can_issue_entries", canIssueEntries, true.B, 0, params.numEntries, 1)
  val isBlocked = PopCount(statusArray.map(s => s.valid && s.blocked))
  RVCOREPerfAccumulate("blocked_entries", isBlocked)
  val isScheduled = PopCount(statusArray.map(s => s.valid && s.scheduled))
  RVCOREPerfAccumulate("scheduled_entries", isScheduled)
  val notSelected = RegNext(PopCount(io.canIssue)) - PopCount(is_issued)
  RVCOREPerfAccumulate("not_selected_entries", notSelected)
  val isReplayed = PopCount(io.deqResp.map(resp => resp.valid && !resp.bits.success))
  RVCOREPerfAccumulate("replayed_entries", isReplayed)
}
