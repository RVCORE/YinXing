...
class PayloadArray[T <: Data](gen: T, params: RSParams)(implicit p: Parameters) extends RVCOREModule {
  ...
  io.read.map(_.data).zip(io.read.map(_.addr)).map {
    ...
    RVCOREError(PopCount(addr) > 1.U, p"raddr ${Binary(addr)} is not one-hot\n")
  }
  ...
  for (i <- 0 until params.numEntries) {
    ...
    RVCOREError(PopCount(wenVec) > 1.U, p"wenVec ${Binary(wenVec.asUInt)} is not one-hot\n")
  }

  for (w <- io.write) {
    // check for writing to multiple entries
    RVCOREError(w.enable && PopCount(w.addr.asBools) =/= 1.U,
      p"write address ${Binary(w.addr)} is not one-hot\n")
    // write log
    RVCOREDebug(w.enable, p"write to address ${OHToUInt(w.addr)}\n")
  }
