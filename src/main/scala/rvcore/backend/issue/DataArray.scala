...
import rvcore._
import utils._
import rvcore.backend.decode.{ImmUnion, Imm_LUI_LOAD, Imm_U}
import rvcore.backend.exu.ExuConfig

class DataArrayReadIO(numEntries: Int, numSrc: Int, dataBits: Int)(implicit p: Parameters) extends RVCOREBundle {
  val addr = Input(UInt(numEntries.W))
  val data = Vec(numSrc, Output(UInt(dataBits.W)))

}

class DataArrayWriteIO(numEntries: Int, numSrc: Int, dataBits: Int)(implicit p: Parameters) extends RVCOREBundle {
...
class DataArrayMultiWriteIO(numEntries: Int, numSrc: Int, dataBits: Int)(implicit p: Parameters) extends RVCOREBundle {
  val enable = Input(Bool())
...
class DataArrayIO(params: RSParams)(implicit p: Parameters) extends RVCOREBundle {
...
class DataArray(params: RSParams)(implicit p: Parameters) extends RVCOREModule {
  ...
  for (i <- 0 until params.numSrc) {
    ...
    for (i <- 0 until params.numEntries) {
      ...
      when(PopCount(w) > 1.U) {
        RVCOREDebug("ERROR: RS DataArray write overlap!\n")
      }
    }
  }
...
class ImmExtractor(numSrc: Int, dataBits: Int)(implicit p: Parameters) extends RVCOREModule {
