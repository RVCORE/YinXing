...
class WakeupQueue(number: Int)(implicit p: Parameters) extends RVCOREModule {
  ...
  if (number < 0) {
    ...
    for (i <- 0 until number) {
      RVCOREDebug(queue(i).valid, p"BPQue(${i.U}): pc:${Hexadecimal(queue(i).bits.cf.pc)} robIdx:${queue(i).bits.robIdx}" +
