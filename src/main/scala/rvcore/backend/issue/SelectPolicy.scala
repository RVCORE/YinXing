...
class SelectPolicy(params: RSParams)(implicit p: Parameters) extends RVCOREModule {
  ...
  for (i <- 0 until params.numEnq) {
    ...
    RVCOREError(io.allocate(i).valid && PopCount(io.allocate(i).bits) =/= 1.U,
      p"allocate vec ${Binary(io.allocate(i).bits)} is not onehot")
    RVCOREDebug(io.allocate(i).fire(), p"select for allocation: ${Binary(io.allocate(i).bits)}\n")
  }
  ...
  for (i <- 0 until params.numDeq) {
    ...
    RVCOREError(io.grant(i).valid && PopCount(io.grant(i).bits.asBools) =/= 1.U,
      p"grant vec ${Binary(io.grant(i).bits)} is not onehot")
    RVCOREDebug(io.grant(i).valid, p"select for issue request: ${Binary(io.grant(i).bits)}\n")
  }
  io.grantBalance := select.getBalance2

}

class OldestSelection(params: RSParams)(implicit p: Parameters) extends RVCOREModule {
  val io = IO(new Bundle() {
  ...
  for ((out, i) <- io.out.zipWithIndex) {
    ...
    RVCOREPerfAccumulate(s"oldest_override_$i", io.isOverrided(i))
  }
}

class AgeDetector(numEntries: Int, numEnq: Int, regOut: Boolean = true)(implicit p: Parameters) extends RVCOREModule {
  val io = IO(new Bundle {
  ...
  RVCOREError(VecInit(age.map(v => VecInit(v).asUInt.andR)).asUInt =/= RegNext(nextBest), "age error\n")
}
