...
import rvcore.backend.fu.FuConfig
import rvcore.mem.{SqPtr, MemWaitUpdateReq}
import rvcore.backend.fu.fpu.{FMAMidResult, FMAMidResultIO}
...
class ReservationStationWrapper(implicit p: Parameters) extends LazyModule with HasRVCOREParameter {
  ...
  def addIssuePort(cfg: ExuConfig, deq: Int): Unit = {
    ...
    cfg match {
      case JumpCSRExeUnitCfg => params.numEntries += CSRIssQueSize * deq
      case AluExeUnitCfg => params.numEntries += ALUIssQueSize * deq
      case StaExeUnitCfg => params.numEntries += STAIssQueSize * deq
      case StdExeUnitCfg => params.numEntries += STDIssQueSize * deq
      case MulDivExeUnitCfg => params.numEntries += MDUIssQueSize * deq
      case LdExeUnitCfg => params.numEntries += LDIssQueSize * deq
      case FmacExeUnitCfg => params.numEntries += FMACIssQueSize * deq
      case FmiscExeUnitCfg => params.numEntries += FMISCIssQueSize * deq
    }
  ...
  lazy val module = new LazyModuleImp(this) with HasPerfEvents {
    ...
    val rs = (0 until numRS).map(i => {
      ...
      val updatedP = p.alter((site, here, up) => {
        case RVCORECoreParamsKey => up(RVCORECoreParamsKey).copy(
...
class ReservationStationIO(params: RSParams)(implicit p: Parameters) extends RVCOREBundle {
...
class ReservationStation(params: RSParams)(implicit p: Parameters) extends RVCOREModule with HasPerfEvents {
  ...
  generatePerfEvent()
  RVCOREPerfAccumulate("full", statusArray.io.isValid.andR)
  ...
  for (i <- 0 until params.numDeq) {
    ...
    if (io.fastWakeup.isDefined) {
      ...
      RVCOREPerfAccumulate(s"fast_blocked_$i", issueVec(i).valid && fuCheck && !s1_out(i).ready)
    }
  }
  ...
  /**
    ...
    */
  // Do the read data arbitration
  class DataSelect(implicit p: Parameters) extends RVCOREModule {
  ...
  for (i <- 0 until params.numDeq) {
    ...
    if (params.hasMidState) {
      ...
      RVCOREPerfAccumulate(s"fma_partial2_issue_$i", io.deq(i).fire && io.fmaMid.get(i).waitForAdd)
      RVCOREPerfAccumulate(s"fma_final_issue_$i", io.deq(i).fire && io.fmaMid.get(i).in.valid)
    }
    ...
    if (params.numFastWakeup > 0) {
      ...
      if (params.isLoad) {
        if (EnableLoadToLoadForward) {
          ...
          RVCOREPerfAccumulate(s"fast_load_deq_valid_$i", !s2_deq(i).valid && ldFastDeq.valid)
          RVCOREPerfAccumulate(s"fast_load_deq_fire_$i", !s2_deq(i).valid && ldFastDeq.valid && io.deq(i).ready)
        } else {
      ...
      for (j <- 0 until params.numFastWakeup) {
        RVCOREPerfAccumulate(s"source_bypass_${j}_$i", s1_out(i).fire() && wakeupBypassMask(j).asUInt().orR())
      }
    }
  }
  ...
  if (params.hasMidState) {
    ...
    for (i <- 0 until params.numDeq) {
      ...
      RVCOREPerfAccumulate(s"fma_final_issue_cond11_$i", cond11Issued)
      ...
      RVCOREPerfAccumulate(s"fma_final_issue_cond12_$i", cond12Issued)
      ...
      RVCOREPerfAccumulate(s"fma_final_selected_cond2_$i", cond2Selected)
    }
  }
  ...
  for ((dispatch, i) <- io.fromDispatch.zipWithIndex) {
    RVCOREDebug(dispatch.valid && !dispatch.ready, p"enq blocked, robIdx ${dispatch.bits.robIdx}\n")
    RVCOREDebug(dispatch.fire(), p"enq fire, robIdx ${dispatch.bits.robIdx}, srcState ${Binary(dispatch.bits.srcState.asUInt)}\n")
    RVCOREPerfAccumulate(s"allcoate_fire_$i", dispatch.fire())
    RVCOREPerfAccumulate(s"allocate_valid_$i", dispatch.valid)
    RVCOREPerfAccumulate(s"srcState_ready_$i", PopCount(dispatch.bits.srcState.map(_ === SrcState.rdy)))
    if (params.checkWaitBit) {
      RVCOREPerfAccumulate(s"load_wait_$i", dispatch.fire() && dispatch.bits.cf.loadWaitBit)
    }
  }

  for ((deq, i) <- io.deq.zipWithIndex) {
    RVCOREDebug(deq.fire(), p"deq fire, robIdx ${deq.bits.uop.robIdx}\n")
    RVCOREDebug(deq.valid && !deq.ready, p"deq blocked, robIdx ${deq.bits.uop.robIdx}\n")
    RVCOREPerfAccumulate(s"deq_fire_$i", deq.fire())
    RVCOREPerfAccumulate(s"deq_valid_$i", deq.valid)
    if (params.hasFeedback) {
      RVCOREPerfAccumulate(s"deq_not_first_issue_$i", deq.fire() && !io.feedback.get(i).isFirstIssue)
    }
  }

  for (i <- 0 until params.numEntries) {
    val isSelected = VecInit(issueVec.map(s => s.valid && s.bits(i))).asUInt().orR()
    RVCOREPerfAccumulate(s"select_$i", isSelected)
    val isIssued = VecInit(issueVec.zip(s1_out_fire).map(s => s._2 && s._1.bits(i))).asUInt().orR()
    RVCOREPerfAccumulate(s"issue_$i", isIssued)
    for (j <- 0 until params.numSrc) {
      RVCOREPerfAccumulate(s"num_wakeup_${i}_$j", slowWakeupMatchVec(i)(j).asUInt().orR())
    }
  }

  RVCOREPerfAccumulate("redirect_num", io.redirect.valid)
  RVCOREPerfHistogram("allocate_num", PopCount(io.fromDispatch.map(_.valid)), true.B, 0, params.numEnq, 1)
  RVCOREPerfHistogram("issue_num", PopCount(io.deq.map(_.valid)), true.B, 0, params.numDeq, 1)
