...
import utils._
import rvcore._
import rvcore.backend.exu.StdExeUnit
import rvcore.backend.fu._
import rvcore.backend.rob.RobLsqIO
import rvcore.cache._
import rvcore.cache.mmu.{BTlbPtwIO, TLB, TlbReplace}
import rvcore.mem._
...
class MemBlock()(implicit p: Parameters) extends LazyModule
  with HasRVCOREParameter with HasWritebackSource {
  ...
  with HasPerfEvents
{
  ...
  RVCOREDebug(tEnable.asUInt.orR, "Debug Mode: At least one store trigger is enabled\n")
  for(j <- 0 until 3)
...
//      io.writeback(i).bits.uop.cf.trigger.backendTiming(lTriggerMapping(j)) := tdata(lTriggerMapping(j)).timing
    ...
    RVCOREDebug(io.writeback(i).bits.uop.cf.trigger.getHitBackend && io.writeback(i).valid, p"Debug Mode: Load Inst No.${i}" +
...
//    }
  ...
  RVCOREError(atomicsException && atomicsUnit.io.in.valid, "new instruction before exception triggers\n")
  ...
  RVCOREPerfAccumulate("load_rs_deq_count", ldDeqCount)
  RVCOREPerfHistogram("load_rs_deq_count", ldDeqCount, true.B, 0, exuParameters.LduCnt, 1)
  RVCOREPerfAccumulate("store_rs_deq_count", stDeqCount)
  RVCOREPerfHistogram("store_rs_deq_count", stDeqCount, true.B, 0, exuParameters.StuCnt, 1)
  RVCOREPerfAccumulate("ls_rs_deq_count", rsDeqCount)
