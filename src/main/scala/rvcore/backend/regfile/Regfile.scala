...
package rvcore.backend.regfile
...
class RfReadPort(len: Int)(implicit p: Parameters) extends RVCOREBundle {
  val addr = Input(UInt(PhyRegIdxWidth.W))
  val data = Output(UInt(len.W))
}

class RfWritePort(len: Int)(implicit p: Parameters) extends RVCOREBundle {
  val wen = Input(Bool())
  val addr = Input(UInt(PhyRegIdxWidth.W))
...
(
  ...
  hasZero: Boolean,
  len: Int
)(implicit p: Parameters) extends RVCOREModule {
