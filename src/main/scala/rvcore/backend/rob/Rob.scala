...
class RobCSRIO(implicit p: Parameters) extends RVCOREBundle {
...
class RobLsqIO(implicit p: Parameters) extends RVCOREBundle {
...
class RobEnqIO(implicit p: Parameters) extends RVCOREBundle {
...
class RobDeqPtrWrapper(implicit p: Parameters) extends RVCOREModule with HasCircularQueuePtrHelper {
  ...
  when (io.state === 0.U) {
    RVCOREInfo(io.state === 0.U && commitCnt > 0.U, "retired %d insts\n", commitCnt)
  }

}

class RobEnqPtrWrapper(implicit p: Parameters) extends RVCOREModule with HasCircularQueuePtrHelper {
  val io = IO(new Bundle {
    // for input redirect
...
class RobExceptionInfo(implicit p: Parameters) extends RVCOREBundle {
  // val valid = Bool()
...
class ExceptionGen(implicit p: Parameters) extends RVCOREModule with HasCircularQueuePtrHelper {
  val io = IO(new Bundle {
    val redirect = Input(Valid(new Redirect))
...
class RobFlushInfo(implicit p: Parameters) extends RVCOREBundle {
...
class Rob(implicit p: Parameters) extends LazyModule with HasWritebackSink with HasRVCOREParameter {
...
class RobImp(outer: Rob)(implicit p: Parameters) extends LazyModuleImp(outer)
  with HasRVCOREParameter with HasCircularQueuePtrHelper with HasPerfEvents {
  ...
  RVCOREDebug(p"(ready, valid): ${io.enq.canAccept}, ${Binary(Cat(io.enq.req.map(_.valid)))}\n")
  RVCOREInfo(dispatchNum =/= 0.U, p"dispatched $dispatchNum insts\n")
  ...
  RVCOREInfo(writebackNum =/= 0.U, "writebacked %d insts\n", writebackNum)
  ...
  RVCOREDebug(deqHasException && exceptionDataRead.bits.singleStep, "Debug Mode: Deq has singlestep exception\n")
  RVCOREDebug(deqHasException && exceptionDataRead.bits.trigger.getHitFrontend, "Debug Mode: Deq has frontend trigger exception\n")
  RVCOREDebug(deqHasException && exceptionDataRead.bits.trigger.getHitBackend, "Debug Mode: Deq has backend trigger exception\n")
  ...
  RVCOREPerfAccumulate("interrupt_num", io.flushOut.valid && intrEnable)
  RVCOREPerfAccumulate("exception_num", io.flushOut.valid && exceptionEnable)
  RVCOREPerfAccumulate("flush_pipe_num", io.flushOut.valid && isFlushPipe)
  RVCOREPerfAccumulate("replay_inst_num", io.flushOut.valid && isFlushPipe && deqHasReplayInst)
  ...
  for (i <- 0 until CommitWidth) {
    ...
    RVCOREInfo(state === s_idle && io.commits.valid(i),
    ...
    RVCOREInfo(state === s_walk && io.commits.valid(i), "walked pc %x wen %d ldst %d data %x\n",
      debug_microOp(walkPtrVec(i).value).cf.pc,
      io.commits.info(i).rfWen,
      io.commits.info(i).ldest,
      debug_exuData(walkPtrVec(i).value)
    )
    RVCOREInfo(state === s_extrawalk && io.commits.valid(i), "use extra space walked wen %d ldst %d\n",
      io.commits.info(i).rfWen,
      io.commits.info(i).ldest
    )
  }
  ...
  when (io.redirect.valid) {
    ...
    RVCOREInfo(p"rolling back: $enqPtr $deqPtr walk $walkPtr walkcnt $walkCounter\n")
  }
  ...
  /**
    * debug info
    */
  RVCOREDebug(p"enqPtr ${enqPtr} deqPtr ${deqPtr}\n")
  RVCOREDebug("")
  for(i <- 0 until RobSize){
    RVCOREDebug(false, !valid(i), "-")
    RVCOREDebug(false, valid(i) && writebacked(i), "w")
    RVCOREDebug(false, valid(i) && !writebacked(i), "v")
  }
  RVCOREDebug(false, true.B, "\n")

  for(i <- 0 until RobSize) {
    if(i % 4 == 0) RVCOREDebug("")
    RVCOREDebug(false, true.B, "%x ", debug_microOp(i).cf.pc)
    RVCOREDebug(false, !valid(i), "- ")
    RVCOREDebug(false, valid(i) && writebacked(i), "w ")
    RVCOREDebug(false, valid(i) && !writebacked(i), "v ")
    if(i % 4 == 3) RVCOREDebug(false, true.B, "\n")
  }
  ...
  RVCOREPerfAccumulate("clock_cycle", 1.U)
  QueuePerf(RobSize, PopCount((0 until RobSize).map(valid(_))), !allowEnqueue)
  RVCOREPerfAccumulate("commitUop", ifCommit(commitCnt))
  RVCOREPerfAccumulate("commitInstr", ifCommit(trueCommitCnt))
  val commitIsMove = commitDebugUop.map(_.ctrl.isMove)
  RVCOREPerfAccumulate("commitInstrMove", ifCommit(PopCount(io.commits.valid.zip(commitIsMove).map{ case (v, m) => v && m })))
  val commitMoveElim = commitDebugUop.map(_.debugInfo.eliminatedMove)
  RVCOREPerfAccumulate("commitInstrMoveElim", ifCommit(PopCount(io.commits.valid zip commitMoveElim map { case (v, e) => v && e })))
  RVCOREPerfAccumulate("commitInstrFused", ifCommit(fuseCommitCnt))
  ...
  RVCOREPerfAccumulate("commitInstrLoad", ifCommit(PopCount(commitLoadValid)))
  ...
  RVCOREPerfAccumulate("commitInstrBranch", ifCommit(PopCount(commitBranchValid)))
  val commitLoadWaitBit = commitDebugUop.map(_.cf.loadWaitBit)
  RVCOREPerfAccumulate("commitInstrLoadWait", ifCommit(PopCount(commitLoadValid.zip(commitLoadWaitBit).map{ case (v, w) => v && w })))
  ...
  RVCOREPerfAccumulate("waitNormalCycle", deqNotWritebacked && deqUopCommitType === CommitType.NORMAL)
  RVCOREPerfAccumulate("waitBranchCycle", deqNotWritebacked && deqUopCommitType === CommitType.BRANCH)
  RVCOREPerfAccumulate("waitLoadCycle", deqNotWritebacked && deqUopCommitType === CommitType.LOAD)
  RVCOREPerfAccumulate("waitStoreCycle", deqNotWritebacked && deqUopCommitType === CommitType.STORE)
  RVCOREPerfAccumulate("robHeadPC", io.commits.info(0).pc)
  ...
  for (fuType <- FuType.functionNameMap.keys) {
    ...
    RVCOREPerfAccumulate(s"${fuName}_instr_cnt", ifCommit(PopCount(commitIsFuType)))
    RVCOREPerfAccumulate(s"${fuName}_latency_dispatch", ifCommit(latencySum(commitIsFuType, dispatchLatency)))
    RVCOREPerfAccumulate(s"${fuName}_latency_enq_rs", ifCommit(latencySum(commitIsFuType, enqRsLatency)))
    RVCOREPerfAccumulate(s"${fuName}_latency_select", ifCommit(latencySum(commitIsFuType, selectLatency)))
    RVCOREPerfAccumulate(s"${fuName}_latency_issue", ifCommit(latencySum(commitIsFuType, issueLatency)))
    ...
    if (fuType == FuType.fmac.litValue()) {
      ...
      RVCOREPerfAccumulate(s"${fuName}_latency_execute_fma", ifCommit(latencySum(commitIsFma, executeLatency)))
    }
  }
  ...
  // Always instantiate basic difftest modules.
  if (env.EnableDifftest) {
    val dt_isRVCORETrap = Mem(RobSize, Bool())
    for (i <- 0 until RenameWidth) {
      when (canEnqueue(i)) {
        dt_isRVCORETrap(enqPtrVec(i).value) := io.enq.req(i).bits.ctrl.isRVCORETrap
      }
    }
    val trapVec = io.commits.valid.zip(deqPtrVec).map{ case (v, d) => state === s_idle && v && dt_isRVCORETrap(d.value) }
