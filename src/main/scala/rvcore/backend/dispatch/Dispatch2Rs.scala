  ...
  for ((config, i) <- outer.exuConfigCases) {
    ...
    for ((idx, j) <- outIndices.zipWithIndex) {
      ...
      if (config.contains(StdExeUnitCfg)) {
        ...
        RVCOREPerfAccumulate(s"st_rs_not_ready_$idx", selectValid && (!sta.ready || !io.out(idx).ready))
        RVCOREPerfAccumulate(s"sta_rs_not_ready_$idx", selectValid && !sta.ready && io.out(idx).ready)
        RVCOREPerfAccumulate(s"std_rs_not_ready_$idx", selectValid && sta.ready && !io.out(idx).ready)
      }
      else {
