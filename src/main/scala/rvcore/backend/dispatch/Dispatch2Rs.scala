...
import rvcore.backend.rename.BusyTableReadIO
import rvcore.mem.LsqEnqIO

class Dispatch2Rs(val configs: Seq[Seq[ExuConfig]])(implicit p: Parameters) extends LazyModule with HasRVCOREParameter {
...
class Dispatch2RsImp(outer: Dispatch2Rs)(implicit p: Parameters) extends LazyModuleImp(outer) with HasRVCOREParameter {
  ...
  // RVCOREError(numStaFire =/= numStdFire, "sta_fire != std_fire\n")
  val numOutFire = PopCount(io.out.map(_.fire)) - numStdFire
  // RVCOREError(numInFire =/= numOutFire, "in != out\n")

  RVCOREPerfAccumulate("in_valid", PopCount(io.in.map(_.valid)))
  RVCOREPerfAccumulate("in_fire", PopCount(io.in.map(_.fire)))
  RVCOREPerfAccumulate("out_valid", PopCount(io.out.map(_.valid)))
  RVCOREPerfAccumulate("out_fire", PopCount(io.out.map(_.fire)))
}
  ...
  for ((config, i) <- outer.exuConfigCases) {
    ...
    for ((idx, j) <- outIndices.zipWithIndex) {
      ...
      if (config.contains(StdExeUnitCfg)) {
        ...
        RVCOREPerfAccumulate(s"st_rs_not_ready_$idx", selectValid && (!sta.ready || !s0_out(idx).ready))
        RVCOREPerfAccumulate(s"sta_rs_not_ready_$idx", selectValid && !sta.ready && s0_out(idx).ready)
        RVCOREPerfAccumulate(s"std_rs_not_ready_$idx", selectValid && sta.ready && !s0_out(idx).ready)
      }
      else {
