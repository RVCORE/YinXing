...
package rvcore.backend.dispatch
...
import rvcore._
import rvcore.backend.rob.RobEnqIO
...
class Dispatch(implicit p: Parameters) extends RVCOREModule with HasPerfEvents {
  ...
  RVCOREDebug(singleStepStatus, "Debug Mode: Singlestep status is asserted\n")
  ...
  for (i <- 0 until RenameWidth) {
    ...
    when (io.fromRename(i).fire()) {
      RVCOREDebug(updatedUop(i).cf.trigger.getHitFrontend, s"Debug Mode: inst ${i} has frontend trigger exception\n")
      RVCOREDebug(updatedUop(i).ctrl.singleStep, s"Debug Mode: inst ${i} has single step exception\n")
    }
  ...
  RVCOREPerfAccumulate("waittable_load_wait", PopCount((0 until RenameWidth).map(i =>
    io.fromRename(i).fire() && io.fromRename(i).bits.cf.loadWaitBit && !isStore(i) && isLs(i)
  )))
  RVCOREPerfAccumulate("storeset_load_wait", PopCount((0 until RenameWidth).map(i =>
    io.fromRename(i).fire() && updatedUop(i).cf.loadWaitBit && !isStore(i) && isLs(i)
  )))
  RVCOREPerfAccumulate("storeset_load_strict_wait", PopCount((0 until RenameWidth).map(i =>
    io.fromRename(i).fire() && updatedUop(i).cf.loadWaitBit && updatedUop(i).cf.loadWaitStrict && !isStore(i) && isLs(i)
  )))
  RVCOREPerfAccumulate("storeset_store_wait", PopCount((0 until RenameWidth).map(i =>
  ...
  for (i <- 0 until RenameWidth) {
    ...
    RVCOREDebug(io.enqRob.req(i).valid, p"pc 0x${Hexadecimal(io.fromRename(i).bits.cf.pc)} receives nrob ${io.enqRob.resp(i)}\n")
    ...
    RVCOREDebug(io.toIntDq.req(i).valid, p"pc 0x${Hexadecimal(io.toIntDq.req(i).bits.cf.pc)} int index $i\n")
    RVCOREDebug(io.toFpDq.req(i).valid , p"pc 0x${Hexadecimal(io.toFpDq.req(i).bits.cf.pc )} fp  index $i\n")
    RVCOREDebug(io.toLsDq.req(i).valid , p"pc 0x${Hexadecimal(io.toLsDq.req(i).bits.cf.pc )} ls  index $i\n")
  }
  ...
  for (i <- 0 until RenameWidth) {
    ...
    RVCOREInfo(io.recv(i) && io.fromRename(i).valid,
  ...
  RVCOREError(enqFireCnt > renameFireCnt, "enqFireCnt should not be greater than renameFireCnt\n")

  RVCOREPerfAccumulate("in", Mux(RegNext(io.fromRename(0).ready), PopCount(io.fromRename.map(_.valid)), 0.U))
  RVCOREPerfAccumulate("empty", !hasValidInstr)
  RVCOREPerfAccumulate("utilization", PopCount(io.fromRename.map(_.valid)))
  RVCOREPerfAccumulate("waitInstr", PopCount((0 until RenameWidth).map(i => io.fromRename(i).valid && !io.recv(i))))
  RVCOREPerfAccumulate("stall_cycle_rob", hasValidInstr && !io.enqRob.canAccept && io.toIntDq.canAccept && io.toFpDq.canAccept && io.toLsDq.canAccept)
  RVCOREPerfAccumulate("stall_cycle_int_dq", hasValidInstr && io.enqRob.canAccept && !io.toIntDq.canAccept && io.toFpDq.canAccept && io.toLsDq.canAccept)
  RVCOREPerfAccumulate("stall_cycle_fp_dq", hasValidInstr && io.enqRob.canAccept && io.toIntDq.canAccept && !io.toFpDq.canAccept && io.toLsDq.canAccept)
  RVCOREPerfAccumulate("stall_cycle_ls_dq", hasValidInstr && io.enqRob.canAccept && io.toIntDq.canAccept && io.toFpDq.canAccept && !io.toLsDq.canAccept)
