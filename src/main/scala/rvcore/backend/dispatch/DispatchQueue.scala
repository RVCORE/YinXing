...
class DispatchQueueIO(enqnum: Int, deqnum: Int)(implicit p: Parameters) extends RVCOREBundle {
  val enq = new Bundle {
...
class DispatchQueue(size: Int, enqnum: Int, deqnum: Int)(implicit p: Parameters)
  extends RVCOREModule with HasCircularQueuePtrHelper with HasPerfEvents {
  ...
  for (i <- 0 until enqnum) {
    when(io.enq.req(i).valid && canActualEnqueue) {
      ...
      RVCOREError(sel =/= PopCount(io.enq.req.take(i).map(_.valid)), "why not continuous??\n")
    }
  }
  ...
  for (i <- 0 until deqnum) {
    when(io.deq(i).fire() && !io.redirect.valid) {
      ...
      // RVCOREError(stateEntries(headPtr(i).value) =/= s_valid, "state of the dispatch entry is not s_valid\n")
    }
  }
  ...
  for (i <- 0 until size) {
    ...
    RVCOREInfo(needCancel(i), p"valid entry($i)(pc = ${Hexadecimal(debug_uopEntries(i).cf.pc)}) " +
  ...
  RVCOREDebug(p"head: ${headPtr(0)}, tail: ${tailPtr(0)}\n")
  RVCOREDebug(p"state: ")
  stateEntries.reverse.foreach { s =>
    RVCOREDebug(false, s === s_invalid, "-")
    RVCOREDebug(false, s === s_valid, "v")
  }
  RVCOREDebug(false, true.B, "\n")
  RVCOREDebug(p"ptr:   ")
  (0 until size).reverse.foreach { i =>
    val isPtr = i.U === headPtr(0).value || i.U === tailPtr(0).value
    RVCOREDebug(false, isPtr, "^")
    RVCOREDebug(false, !isPtr, " ")
  }
  RVCOREDebug(false, true.B, "\n")

  // RVCOREError(isAfter(headPtr(0), tailPtr(0)), p"assert greaterOrEqualThan(tailPtr: ${tailPtr(0)}, headPtr: ${headPtr(0)}) failed\n")
  QueuePerf(size, PopCount(stateEntries.map(_ =/= s_invalid)), !canEnqueue)
  io.dqFull := !canEnqueue
  RVCOREPerfAccumulate("in", numEnq)
  RVCOREPerfAccumulate("out", PopCount(io.deq.map(_.fire())))
  RVCOREPerfAccumulate("out_try", PopCount(io.deq.map(_.valid)))
  val fake_block = currentValidCounter <= (size - enqnum).U && !canEnqueue
  RVCOREPerfAccumulate("fake_block", fake_block)
