...
class DispatchQueue(size: Int, enqnum: Int, deqnum: Int)(implicit p: Parameters)
  ...
  for (i <- 0 until size) {
    ...
    RVCOREInfo(needCancel(i), p"valid entry($i)(pc = ${Hexadecimal(data(i).cf.pc)}) " +
  ...
  RVCOREError(headPtrOH =/= headPtr.head.toOH, p"head: $headPtrOH != UIntToOH(${headPtr.head})")
  ...
  RVCOREError(tailPtrOHAccurate && tailPtrOH =/= tailPtr.head.toOH, p"tail: $tailPtrOH != UIntToOH(${tailPtr.head})")
