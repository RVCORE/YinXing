...
class FusionDecoder(implicit p: Parameters) extends RVCOREModule {
  ...
  instrPairs.zip(csPairs).zip(io.out).zipWithIndex.foreach{ case (((pair, cs), out), i) =>
    ...
    RVCOREError(PopCount(fusionVec) > 1.U, "more then one fusion matched\n")
    ...
    fusionList.zip(fusionVec).foreach { case (f, v) =>
      RVCOREPerfAccumulate(s"case_${f.fusionName}_$i", pairValid && !thisCleared && v && out.ready)
    }
    RVCOREPerfAccumulate(s"conflict_fusion_$i", pairValid && thisCleared && fusionVec.asUInt().orR() && out.ready)
  }

  RVCOREPerfAccumulate("fused_instr", PopCount(io.out.map(_.fire)))
}
