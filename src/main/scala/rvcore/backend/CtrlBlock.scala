...
import utils._
import rvcore._
import rvcore.backend.decode.{DecodeStage, ImmUnion}
import rvcore.backend.dispatch.{Dispatch, DispatchQueue}
import rvcore.backend.fu.PFEvent
import rvcore.backend.rename.{Rename, RenameTableWrapper}
import rvcore.backend.rob.{Rob, RobCSRIO, RobLsqIO}
import rvcore.frontend.FtqRead
import rvcore.mem.mdp.{LFST, SSIT, WaitTable}

class CtrlToFtqIO(implicit p: Parameters) extends RVCOREBundle {
...
class RedirectGenerator(implicit p: Parameters) extends RVCOREModule
  with HasCircularQueuePtrHelper {

  class RedirectGeneratorIO(implicit p: Parameters) extends RVCOREBundle {
    def numRedirect = exuParameters.JmpCnt + exuParameters.AluCnt
