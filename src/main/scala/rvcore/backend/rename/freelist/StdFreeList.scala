...
class StdFreeList(size: Int)(implicit p: Parameters) extends BaseFreeList(size) with HasPerfEvents {
  ...
  RVCOREError(headPtr.toOH =/= headPtrOH, p"wrong one-hot reg between $headPtr and $headPtrOH")
  ...
  for (i <- 0 until CommitWidth) {
    ...
    RVCOREDebug(io.freeReq(i), p"req#$i free physical reg: ${io.freePhyReg(i)}\n")
  }
  ...
  RVCOREDebug(p"freeRegCnt: $freeRegCnt\n")
  ...
  for(i <- 0 until RenameWidth){
    ...
    RVCOREDebug(p"req:${io.allocateReq(i)} canAllocate:${io.canAllocate} pdest:${io.allocatePhyReg(i)}\n")
  }
  ...
  RVCOREDebug(p"head:$headPtr tail:$tailPtr\n")
  ...
  if (enableFreeListCheck) {
    for (i <- 0 until size) {
      for (j <- i+1 until size) {
        RVCOREError(freeList(i) === freeList(j), s"Found same entry in free list! (i=$i j=$j)\n")
      }
    }
  }

  RVCOREPerfAccumulate("utilization", freeRegCnt)
  RVCOREPerfAccumulate("allocation_blocked", !io.canAllocate)
  RVCOREPerfAccumulate("can_alloc_wrong", !io.canAllocate && freeRegCnt >= RenameWidth.U)
