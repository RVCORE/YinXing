...
class RefCounter(size: Int)(implicit p: Parameters) extends RVCOREModule {
  ...
  for ((de, i) <- deallocate.zipWithIndex) {
    ...
    RVCOREError(RegNext(isFreed) =/= isFreed1, p"why isFreed ${RegNext(isFreed)} $isFreed1\n")
  ...
  for (i <- 1 until size) {
    ...
    RVCOREError(refCounterInc(i) =/= numAlloc1, p"why numAlloc ${refCounterInc(i)} $numAlloc1??")
    RVCOREError(refCounterDec(i) =/= numDealloc1, p"why numDealloc ${refCounterDec(i)} $numDealloc1??")
    refCounterNext(i) := refCounter(i) + refCounterInc(i) - refCounterDec(i)
    RVCOREError(RegNext(refCounter(i) + refCounterInc(i) < refCounterDec(i)), p"why $i?\n")
    refCounter(i) := refCounterNext(i)
  }

  for (i <- 0 until RobSize) {
    val numCounters = PopCount(refCounter.map(_ === i.U))
    RVCOREPerfAccumulate(s"ref_counter_$i", numCounters)
  }
  for (i <- 0 until size) {
    val isFreed = io.freeRegs.map(f => f.valid && f.bits === i.U)
    RVCOREPerfAccumulate(s"free_reg_$i", VecInit(isFreed).asUInt.orR)
  }
}
