...
import rvcore.backend.decode.{ImmUnion, XDecode}
import rvcore.mem.{LqPtr, SqPtr}
import rvcore.frontend.PreDecodeInfo
import rvcore.frontend.HasBPUParameter
import rvcore.frontend.{GlobalHistory, ShiftingGlobalHistory, CircularGlobalHistory, AllFoldedHistories}
import rvcore.frontend.RASEntry
import rvcore.frontend.BPUCtrl
import rvcore.frontend.FtqPtr
import rvcore.frontend.CGHPtr
import rvcore.frontend.FtqRead
import rvcore.frontend.FtqToCtrlIO
import utils._
...
import rvcore.backend.fu.PMPEntry
import rvcore.frontend.Ftq_Redirect_SRAMEntry
import rvcore.frontend.AllFoldedHistories
import rvcore.frontend.AllAheadFoldedHistoryOldestBits
...
class PredictorAnswer(implicit p: Parameters) extends RVCOREBundle {
...
class CfiUpdateInfo(implicit p: Parameters) extends RVCOREBundle with HasBPUParameter {
  // from backend
...
class CtrlFlow(implicit p: Parameters) extends RVCOREBundle {
  val instr = UInt(32.W)
  val pc = UInt(VAddrBits.W)
...
class FPUCtrlSignals(implicit p: Parameters) extends RVCOREBundle {
...
class CtrlSignals(implicit p: Parameters) extends RVCOREBundle {
...
class CfCtrl(implicit p: Parameters) extends RVCOREBundle {
  val cf = new CtrlFlow
  val ctrl = new CtrlSignals
}

class PerfDebugInfo(implicit p: Parameters) extends RVCOREBundle {
...
// Separate LSQ
class LSIdx(implicit p: Parameters) extends RVCOREBundle {
  val lqIdx = new LqPtr
  val sqIdx = new SqPtr
}
...
class MicroOpRbExt(implicit p: Parameters) extends RVCOREBundleWithMicroOp {
  val flag = UInt(1.W)
}

class Redirect(implicit p: Parameters) extends RVCOREBundle {
  val robIdx = new RobPtr
...
class Dp1ToDp2IO(implicit p: Parameters) extends RVCOREBundle {
...
class ResetPregStateReq(implicit p: Parameters) extends RVCOREBundle {
...
class DebugBundle(implicit p: Parameters) extends RVCOREBundle {
  val isMMIO = Bool()
  val isPerfCnt = Bool()
...
class ExuInput(implicit p: Parameters) extends RVCOREBundleWithMicroOp {
  val src = Vec(3, UInt(XLEN.W))
}

class ExuOutput(implicit p: Parameters) extends RVCOREBundleWithMicroOp {
...
class ExternalInterruptIO(implicit p: Parameters) extends RVCOREBundle {
  val mtip = Input(Bool())
  val msip = Input(Bool())
  val meip = Input(Bool())
  val seip = Input(Bool())
...
class CSRSpecialIO(implicit p: Parameters) extends RVCOREBundle {
...
class ExceptionInfo(implicit p: Parameters) extends RVCOREBundleWithMicroOp {
...
class RSFeedback(implicit p: Parameters) extends RVCOREBundle {
...
class MemRSFeedbackIO(implicit p: Parameters) extends RVCOREBundle {
...
class FrontendToCtrlIO(implicit p: Parameters) extends RVCOREBundle {
  // to backend end
...
class TlbCsrBundle(implicit p: Parameters) extends RVCOREBundle {
  val satp = new Bundle {
    val changed = Bool()
...
class SfenceBundle(implicit p: Parameters) extends RVCOREBundle {
  val valid = Bool()
  val bits = new Bundle {
    val rs1 = Bool()
    val rs2 = Bool()
...
class MemPredUpdateReq(implicit p: Parameters) extends RVCOREBundle  {
  val valid = Bool()
...
class CustomCSRCtrlIO(implicit p: Parameters) extends RVCOREBundle {
  // Prefetcher
...
class DistributedCSRUpdateReq(implicit p: Parameters) extends RVCOREBundle {
...
class L1CacheErrorInfo(implicit p: Parameters) extends RVCOREBundle {
...
//class TriggerCf (implicit p: Parameters) extends RVCOREBundle {
...
class FrontendTdataDistributeIO(implicit p: Parameters)  extends RVCOREBundle {
...
class MemTdataDistributeIO(implicit p: Parameters)  extends RVCOREBundle {
  val t = Valid(new Bundle {
...
class MatchTriggerIO(implicit p: Parameters) extends RVCOREBundle {
