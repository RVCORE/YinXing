...
import rvcore.backend.rob.{RobLsqIO, RobPtr}
import difftest._
import device.RAMHelper

class SqPtr(implicit p: Parameters) extends CircularQueuePtr[SqPtr](
  p => p(RVCORECoreParamsKey).StoreQueueSize
){
}
...
class SqEnqIO(implicit p: Parameters) extends RVCOREBundle {
...
// Store Queue
class StoreQueue(implicit p: Parameters) extends RVCOREModule
  ...
  for (i <- 0 until io.enq.req.length) {
    ...
    when (canEnqueue(i) && !enqCancel(i)) {
      ...
      RVCOREError(!io.enq.canAccept || !io.enq.lqCanAccept, s"must accept $i\n")
      RVCOREError(index =/= sqIdx.value, s"must be the same entry $i\n")
    }
    io.enq.resp(i) := sqIdx
  }
  ...
  for (i <- 0 until StorePipelineWidth) {
    ...
    when (io.storeIn(i).fire()) {
      ...
      RVCOREInfo("store addr write to sq idx %d pc 0x%x miss:%d vaddr %x paddr %x mmio %x\n",
  ...
  for (i <- 0 until StorePipelineWidth) {
    ...
    when (io.storeDataIn(i).fire()) {
      ...
      RVCOREInfo("store data write to sq idx %d pc 0x%x data %x -> %x\n",
  ...
  for (i <- 0 until LoadPipelineWidth) {
    ...
    RVCOREDebug(p"$i f1 ${Binary(canForward1)} f2 ${Binary(canForward2)} " +
    ...
    when (vaddrMatchFailed) {
      RVCOREInfo("vaddrMatchFailed: pc %x pmask %x vmask %x\n",
    ...
    RVCOREPerfAccumulate("vaddr_match_failed", vpmaskNotEqual)
    RVCOREPerfAccumulate("vaddr_match_really_failed", vaddrMatchFailed)
  ...
  /**
    ...
    */
  RVCOREError(uncacheState =/= s_idle && uncacheState =/= s_wait && commitCount > 0.U,
  ...
  for (i <- 0 until EnsbufferWidth) {
    ...
    when (io.sbuffer(i).fire()) {
      ...
      RVCOREDebug("sbuffer "+i+" fire: ptr %d\n", ptr)
    }
  }
  ...
  RVCOREPerfAccumulate("mmio_wb_success", io.mmioStout.fire())
  RVCOREPerfAccumulate("mmio_wb_blocked", io.mmioStout.valid && !io.mmioStout.ready)
  RVCOREPerfAccumulate("validEntryCnt", distanceBetween(enqPtrExt(0), deqPtrExt(0)))
  RVCOREPerfAccumulate("cmtEntryCnt", distanceBetween(cmtPtrExt(0), deqPtrExt(0)))
  RVCOREPerfAccumulate("nCmtEntryCnt", distanceBetween(enqPtrExt(0), cmtPtrExt(0)))
  ...
  for (i <- 0 until StoreQueueSize) {
    RVCOREDebug(i + ": pc %x va %x pa %x data %x ",
      uop(i).cf.pc,
      debug_vaddr(i),
      debug_paddr(i),
      debug_data(i)
