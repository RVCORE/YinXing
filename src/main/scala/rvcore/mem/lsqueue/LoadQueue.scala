...
import utils._
import rvcore._
import rvcore.backend.fu.fpu.FPU
...
trait HasLoadHelper { this: RVCOREModule =>
...
class LqEnqIO(implicit p: Parameters) extends RVCOREBundle {
...
class LqTriggerIO(implicit p: Parameters) extends RVCOREBundle {
  val hitLoadAddrTriggerHitVec = Input(Vec(3, Bool()))
  val lqLoadAddrTriggerHitVec = Output(Vec(3, Bool()))
}

// Load Queue
class LoadQueue(implicit p: Parameters) extends RVCOREModule
  ...
  with HasPerfEvents
{
  ...
  for (i <- 0 until io.enq.req.length) {
    ...
    when (canEnqueue(i) && !enqCancel(i)) {
      ...
      error(index) := false.B
      RVCOREError(!io.enq.canAccept || !io.enq.sqCanAccept, s"must accept $i\n")
      RVCOREError(index =/= lqIdx.value, s"must be the same entry $i\n")
    }
    io.enq.resp(i) := lqIdx
  }
  ...
  for (i <- 0 until LoadPipelineWidth) {
    ...
    when(io.loadIn(i).fire()) {
      when(io.loadIn(i).bits.miss) {
        RVCOREInfo(io.loadIn(i).valid, "load miss write to lq idx %d pc 0x%x vaddr %x paddr %x data %x mask %x forwardData %x forwardMask: %x mmio %x\n",
        ...
        RVCOREInfo(io.loadIn(i).valid, "load hit write to cbd lqidx %d pc 0x%x vaddr %x paddr %x data %x mask %x forwardData %x forwardMask: %x mmio %x\n",
        io.loadIn(i).bits.uop.lqIdx.asUInt,
        io.loadIn(i).bits.uop.cf.pc,
        io.loadIn(i).bits.vaddr,
        io.loadIn(i).bits.paddr,
  ...
  when(io.refill.valid) {
    RVCOREDebug("miss resp: paddr:0x%x data %x\n", io.refill.bits.addr, io.refill.bits.data)
  }
  ...
  (0 until LoadPipelineWidth).map(i => {
    ...
    when(io.ldout(i).fire()) {
      RVCOREInfo("int load miss write to cbd robidx %d lqidx %d pc 0x%x mmio %x\n",
  ...
  (0 until CommitWidth).map(i => {
    when(commitCount > i.U){
      ...
      RVCOREError(!allocated((deqPtrExt+i.U).value), s"why commit invalid entry $i?\n")
    }
  })
  ...
  def getOldest[T <: RVCOREBundleWithMicroOp](valid: Seq[Bool], bits: Seq[T]): (Seq[Bool], Seq[T]) = {
  ...
  def detectRollback(i: Int) = {
    ...
    RVCOREDebug(lqViolation, p"${Binary(Cat(lqViolationVec))}, $startIndex, $lqViolationIndex\n")
    ...
    RVCOREDebug(wbViolation, p"${Binary(Cat(wbViolationVec))}, $wbViolationUop\n")
    ...
    val load_s1 = Wire(Vec(LoadPipelineWidth, new RVCOREBundleWithMicroOp))
    ...
    RVCOREDebug(l1Violation, p"${Binary(Cat(l1ViolationVec))}, $l1ViolationUop\n")
  ...
  */
  ...
  when (io.uncache.req.fire()) {
    ...
    RVCOREDebug("uncache req: pc %x addr %x data %x op %x mask %x\n",
      uop(deqPtr).cf.pc,
  ...
  when(io.uncache.resp.fire()){
    ...
    RVCOREDebug("uncache resp: data %x\n", io.refill.bits.data)
  }
  ...
  RVCOREPerfAccumulate("rollback", io.rollback.valid) // rollback redirect generated
  RVCOREPerfAccumulate("mmioCycle", uncacheState =/= s_idle) // lq is busy dealing with uncache req
  RVCOREPerfAccumulate("mmioCnt", io.uncache.req.fire())
  RVCOREPerfAccumulate("refill", io.refill.valid)
  RVCOREPerfAccumulate("writeback_success", PopCount(VecInit(io.ldout.map(i => i.fire()))))
  ...
  // debug info
  RVCOREDebug("enqPtrExt %d:%d deqPtrExt %d:%d\n", enqPtrExt(0).flag, enqPtr, deqPtrExt.flag, deqPtr)

  def PrintFlag(flag: Bool, name: String): Unit = {
    when(flag) {
      RVCOREDebug(false, true.B, name)
    }.otherwise {
  ...
  for (i <- 0 until LoadQueueSize) {
    RVCOREDebug(i + " pc %x pa %x ", uop(i).cf.pc, debug_paddr(i))
