...
class StoreUnit_S0(implicit p: Parameters) extends RVCOREModule {
  ...
  RVCOREPerfAccumulate("in_fire_first_issue", io.in.fire && io.isFirstIssue)
  RVCOREPerfAccumulate("addr_spec_success", io.out.fire() && saddr(VAddrBits-1, 12) === io.in.bits.src(0)(VAddrBits-1, 12))
  RVCOREPerfAccumulate("addr_spec_failed", io.out.fire() && saddr(VAddrBits-1, 12) =/= io.in.bits.src(0)(VAddrBits-1, 12))
  RVCOREPerfAccumulate("addr_spec_success_once", io.out.fire() && saddr(VAddrBits-1, 12) === io.in.bits.src(0)(VAddrBits-1, 12) && io.isFirstIssue)
  RVCOREPerfAccumulate("addr_spec_failed_once", io.out.fire() && saddr(VAddrBits-1, 12) =/= io.in.bits.src(0)(VAddrBits-1, 12) && io.isFirstIssue)
}

// Store Pipeline Stage 1
// TLB resp (send paddr to dcache)
class StoreUnit_S1(implicit p: Parameters) extends RVCOREModule {
  val io = IO(new Bundle() {
  ...
  RVCOREDebug(io.rsFeedback.valid,
...
class StoreUnit_S2(implicit p: Parameters) extends RVCOREModule {
  val io = IO(new Bundle() {
    val in = Flipped(Decoupled(new LsPipelineBundle))
...
class StoreUnit_S3(implicit p: Parameters) extends RVCOREModule {
  val io = IO(new Bundle() {
    val in = Flipped(Decoupled(new LsPipelineBundle))
...
class StoreUnit(implicit p: Parameters) extends RVCOREModule {
  val io = IO(new Bundle() {
