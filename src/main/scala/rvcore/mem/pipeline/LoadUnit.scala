...
import rvcore._
import rvcore.backend.fu.PMPRespBundle
import rvcore.cache._
import rvcore.cache.mmu.{TlbCmd, TlbReq, TlbRequestIO, TlbResp}

class LoadToLsqIO(implicit p: Parameters) extends RVCOREBundle {
...
class LoadToLoadIO(implicit p: Parameters) extends RVCOREBundle {
  // load to load fast path is limited to ld (64 bit) used as vaddr src1 only
  val data = UInt(XLEN.W)
  val valid = Bool()
}

class LoadUnitTriggerIO(implicit p: Parameters) extends RVCOREBundle {
...
class LoadUnit_S0(implicit p: Parameters) extends RVCOREModule with HasDCacheParameters{
  ...
  if (EnableLoadToLoadForward) {
    ...
    RVCOREPerfAccumulate("load_to_load_forward", io.loadFastMatch.orR && io.in.fire())
  }
  ...
  RVCOREDebug(io.dcacheReq.fire(),
    p"[DCACHE LOAD REQ] pc ${Hexadecimal(s0_uop.cf.pc)}, vaddr ${Hexadecimal(s0_vaddr)}\n"
  )
  RVCOREPerfAccumulate("in_valid", io.in.valid)
  RVCOREPerfAccumulate("in_fire", io.in.fire)
  RVCOREPerfAccumulate("in_fire_first_issue", io.in.valid && io.isFirstIssue)
  RVCOREPerfAccumulate("stall_out", io.out.valid && !io.out.ready && io.dcacheReq.ready)
  RVCOREPerfAccumulate("stall_dcache", io.out.valid && io.out.ready && !io.dcacheReq.ready)
  RVCOREPerfAccumulate("addr_spec_success", io.out.fire() && s0_vaddr(VAddrBits-1, 12) === io.in.bits.src(0)(VAddrBits-1, 12))
  RVCOREPerfAccumulate("addr_spec_failed", io.out.fire() && s0_vaddr(VAddrBits-1, 12) =/= io.in.bits.src(0)(VAddrBits-1, 12))
  RVCOREPerfAccumulate("addr_spec_success_once", io.out.fire() && s0_vaddr(VAddrBits-1, 12) === io.in.bits.src(0)(VAddrBits-1, 12) && io.isFirstIssue)
  RVCOREPerfAccumulate("addr_spec_failed_once", io.out.fire() && s0_vaddr(VAddrBits-1, 12) =/= io.in.bits.src(0)(VAddrBits-1, 12) && io.isFirstIssue)
}


// Load Pipeline Stage 1
// TLB resp (send paddr to dcache)
class LoadUnit_S1(implicit p: Parameters) extends RVCOREModule {
  val io = IO(new Bundle() {
  ...
  RVCOREPerfAccumulate("in_valid", io.in.valid)
  RVCOREPerfAccumulate("in_fire", io.in.fire)
  RVCOREPerfAccumulate("in_fire_first_issue", io.in.fire && io.in.bits.isFirstIssue)
  RVCOREPerfAccumulate("tlb_miss", io.in.fire && s1_tlb_miss)
  RVCOREPerfAccumulate("tlb_miss_first_issue", io.in.fire && s1_tlb_miss && io.in.bits.isFirstIssue)
  RVCOREPerfAccumulate("stall_out", io.out.valid && !io.out.ready)
}

// Load Pipeline Stage 2
// DCache resp
class LoadUnit_S2(implicit p: Parameters) extends RVCOREModule with HasLoadHelper {
  val io = IO(new Bundle() {
    val in = Flipped(Decoupled(new LsPipelineBundle))
    val out = Decoupled(new LsPipelineBundle)
    val rsFeedback = ValidIO(new RSFeedback)
  ...
  RVCOREDebug(io.out.fire(), "[FWD LOAD RESP] pc %x fwd %x(%b) + %x(%b)\n",
    s2_uop.cf.pc,
  ...
  RVCOREDebug(io.out.fire(), "[DCACHE LOAD RESP] pc %x rdata %x <- D$ %x + fwd %x(%b)\n",
  ...
  RVCOREPerfAccumulate("in_fire", io.in.fire)
  RVCOREPerfAccumulate("in_fire_first_issue", io.in.fire && io.in.bits.isFirstIssue)
  RVCOREPerfAccumulate("dcache_miss", io.in.fire && s2_cache_miss)
  RVCOREPerfAccumulate("dcache_miss_first_issue", io.in.fire && s2_cache_miss && io.in.bits.isFirstIssue)
  RVCOREPerfAccumulate("full_forward", io.in.valid && fullForward)
  RVCOREPerfAccumulate("dcache_miss_full_forward", io.in.valid && s2_cache_miss && fullForward)
  RVCOREPerfAccumulate("replay",  io.rsFeedback.valid && !io.rsFeedback.bits.hit)
  RVCOREPerfAccumulate("replay_tlb_miss", io.rsFeedback.valid && !io.rsFeedback.bits.hit && s2_tlb_miss)
  RVCOREPerfAccumulate("replay_cache", io.rsFeedback.valid && !io.rsFeedback.bits.hit && !s2_tlb_miss && s2_cache_replay)
  RVCOREPerfAccumulate("stall_out", io.out.valid && !io.out.ready)
  RVCOREPerfAccumulate("replay_from_fetch_forward", io.out.valid && forwardFailReplay)
  RVCOREPerfAccumulate("replay_from_fetch_load_vio", io.out.valid && ldldVioReplay)
}

class LoadUnit(implicit p: Parameters) extends RVCOREModule 
  with HasLoadHelper
  with HasPerfEvents
