...
class WaitTable(implicit p: Parameters) extends RVCOREModule {
  ...
  // debug
  when (io.update.valid) {
    RVCOREDebug("%d: waittable update: pc %x data: %x\n", GTimer(), io.update.waddr, io.update.wdata)
  }

  RVCOREPerfAccumulate("wait_table_bit_set", PopCount(data.map(d => d(1))))
}
