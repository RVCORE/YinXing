...
import rvcore.backend.fu.FenceToSbuffer
...
class LsPipelineBundle(implicit p: Parameters) extends RVCOREBundleWithMicroOp {
...
class LoadForwardQueryIO(implicit p: Parameters) extends RVCOREBundleWithMicroOp {
...
class LoadViolationQueryReq(implicit p: Parameters) extends RVCOREBundleWithMicroOp { // provide lqIdx
  val paddr = UInt(PAddrBits.W)
}

class LoadViolationQueryResp(implicit p: Parameters) extends RVCOREBundle {
  val have_violation = Bool()
}

class LoadViolationQueryIO(implicit p: Parameters) extends RVCOREBundle {
...
class MemWaitUpdateReq(implicit p: Parameters) extends RVCOREBundle {
