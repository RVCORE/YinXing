...
import utils.{RVCOREDebug, RVCOREInfo}
import rvcore._
import rvcore.cache.{DCacheLineIO, DCacheWordReq, MemoryOpConstants}

// Fake Store buffer for RVCore Out of Order LSU
class FakeSbuffer(implicit p: Parameters) extends RVCOREModule {
  ...
  RVCOREInfo(io.in(0).fire(), "ensbuffer addr 0x%x wdata 0x%x mask %b\n", io.in(0).bits.addr, io.in(0).bits.data, io.in(0).bits.mask)
  RVCOREInfo(io.in(1).fire(), "ensbuffer addr 0x%x wdata 0x%x mask %b\n", io.in(1).bits.addr, io.in(1).bits.data, io.in(0).bits.mask)
  RVCOREInfo(io.dcache.req.fire(), "desbuffer addr 0x%x wdata 0x%x mask %b\n", io.dcache.req.bits.addr, io.dcache.req.bits.data, io.dcache.req.bits.mask)
}
