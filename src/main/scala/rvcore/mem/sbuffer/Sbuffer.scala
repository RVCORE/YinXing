...
trait HasSbufferConst extends HasRVCOREParameter {
...
class SbufferBundle(implicit p: Parameters) extends RVCOREBundle with HasSbufferConst
...
class SbufferData(implicit p: Parameters) extends RVCOREModule with HasSbufferConst {
...
class Sbuffer(implicit p: Parameters) extends DCacheModule with HasSbufferConst with HasPerfEvents {
  ...
  */
  ...
  RVCOREPerfAccumulate("do_uarch_drain", do_uarch_drain)
  ...
  def mergeWordReq(req: DCacheWordReq, reqptag: UInt, reqvtag: UInt, mergeIdx: UInt, mergeVec: UInt, wordOffset: UInt): Unit = {
    ...
    //        data(entryIdx)(wordOffset)(i) := req.data(i*8+7, i*8)
        ...
        when(reqvtag =/= vtag(entryIdx)) {
          RVCOREDebug("reqvtag =/= sbufvtag req(vtag %x ptag %x) sbuffer(vtag %x ptag %x)\n",
            reqvtag << OffsetWidth,
            reqptag << OffsetWidth,
  ...
  for(((in, wordOffset), i) <- io.in.zip(words).zipWithIndex){
    ...
    when(in.fire()){
      when(canMerge(i)){
        ...
        RVCOREDebug(p"merge req $i to line [${mergeIdx(i)}]\n")
      }.otherwise({
        ...
        RVCOREDebug(p"insert req $i to line[$insertIdx]\n")
  ...
  RVCOREDebug(p"validCount[$validCount]\n")
  ...
  RVCOREDebug(p"sbuffer state:${sbuffer_state} do eviction:${do_eviction} empty:${empty}\n")
  ...
  when(willSendDcacheReq){
    ...
    RVCOREDebug(p"$evictionIdx will be sent to Dcache\n")
  }
  RVCOREDebug(p"need drain:$need_drain cohHasTimeOut: $cohHasTimeOut need replace:$need_replace\n")
  RVCOREDebug(p"drainIdx:$drainIdx tIdx:$cohTimeOutIdx replIdx:$replaceIdx " +
    p"blocked:${!noSameBlockInflight(evictionIdx)} v:${activeMask(evictionIdx)}\n")
  RVCOREDebug(p"prepareValid:$prepareValid evictIdx:$evictionIdx dcache ready:${io.dcache.req.ready}\n")
  ...
  RVCOREDebug(io.dcache.req.fire(),
  ...
  RVCOREPerfAccumulate("vaddr_match_failed", mismatch.reduce(_ || _))
  for ((forward, i) <- io.forward.zipWithIndex) {
    ...
    when (tag_mismatch) {
      RVCOREDebug("forward tag mismatch: pmatch %x vmatch %x vaddr %x paddr %x\n",
  ...
  for (i <- 0 until StoreBufferSize) {
    RVCOREDebug("sbf entry " + i + " : ptag %x vtag %x valid %x active %x inflight %x w_timeout %x\n",
      ptag(i) << OffsetWidth,
      vtag(i) << OffsetWidth,
      stateVec(i).isValid(),
      activeMask(i),
  ...
  RVCOREPerfAccumulate("sbuffer_merge", PopCount(VecInit(io.in.zipWithIndex.map({case (in, i) => in.fire() && canMerge(i)})).asUInt))
  RVCOREPerfAccumulate("sbuffer_newline", PopCount(VecInit(io.in.zipWithIndex.map({case (in, i) => in.fire() && !canMerge(i)})).asUInt))
  RVCOREPerfAccumulate("dcache_req_valid", io.dcache.req.valid)
  RVCOREPerfAccumulate("dcache_req_fire", io.dcache.req.fire())
  RVCOREPerfAccumulate("sbuffer_idle", sbuffer_state === x_idle)
  RVCOREPerfAccumulate("sbuffer_flush", sbuffer_state === x_drain_sbuffer)
  RVCOREPerfAccumulate("sbuffer_replace", sbuffer_state === x_replace)
  (0 until EnsbufferWidth).foreach(i => RVCOREPerfAccumulate(s"canInserts_${i}", canInserts(i)))
  RVCOREPerfAccumulate("mainpipe_resp_valid", io.dcache.main_pipe_hit_resp.fire())
  RVCOREPerfAccumulate("refill_resp_valid", io.dcache.refill_hit_resp.fire())
  RVCOREPerfAccumulate("replay_resp_valid", io.dcache.replay_resp.fire())
  RVCOREPerfAccumulate("coh_timeout", cohHasTimeOut)

  // val (store_latency_sample, store_latency) = TransactionLatencyCounter(io.lsu.req.fire(), io.lsu.resp.fire())
  // RVCOREPerfHistogram("store_latency", store_latency, store_latency_sample, 0, 100, 10)
  // RVCOREPerfAccumulate("store_req", io.lsu.req.fire())
