...
import freechips.rocketchip.tilelink.{BankBinder, TLBuffer, TLIdentityNode, TLXbar}
import huancun.debug.TLLogger
import system.HasSoCParameter
import top.BusPerfMonitor
import utils._
import rvcore.backend._
import rvcore.backend.exu.{ExuConfig, Wb2Ctrl, WbArbiterWrapper}
import rvcore.cache.mmu._
import rvcore.frontend._

import scala.collection.mutable.ListBuffer

abstract class RVCOREModule(implicit val p: Parameters) extends MultiIOModule
...
abstract class RVCOREBundle(implicit val p: Parameters) extends Bundle
  with HasRVCOREParameter

abstract class RVCORECoreBase()(implicit p: config.Parameters) extends LazyModule
  with HasRVCOREParameter with HasExuWbHelper with HasSoCParameter
{
  // interrupt sinks
  ...
  l1d_logger := memBlock.dcache.clientNode


//  val l1d_to_l2_bufferOpt = coreParams.dcacheParametersOpt.map { _ =>
//    val buffer = LazyModule(new TLBuffer)
//    l1d_logger := buffer.node := memBlock.dcache.clientNode
//    buffer
//  }
  ...
  //val l1_xbar_to_l2_buffer = TLIdentityNode()
  //l1_xbar_to_l2_buffer :*= TLBuffer() :*= l1_xbar
...
class RVCORECore()(implicit p: config.Parameters) extends RVCORECoreBase
  with HasRVCOREDts
{
  lazy val module = new RVCORECoreImp(this)
}

class RVCORECoreImp(outer: RVCORECoreBase) extends LazyModuleImp(outer)
  with HasRVCOREParameter
  ...
  val io = IO(new Bundle {
    ...
    val beu_errors = Output(new RVCOREL1BusErrors())
  })
  ...
  RVCOREPerfHistogram("fastIn_count", PopCount(allFastUop1.map(_.valid)), true.B, 0, allFastUop1.length, 1)
  RVCOREPerfHistogram("wakeup_count", PopCount(rfWriteback.map(_.valid)), true.B, 0, rfWriteback.length, 1)
  ...
  // if l2 prefetcher use stream prefetch, it should be placed in RVCORECore
