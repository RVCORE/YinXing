...
trait VMEArgParser {
  ...
  val usage = """
        Usage: sbt "run [OPTION...]"
            -s, --source   the verilog file generated by chisel, all in one file
                           default: $YINXING_HOME/build/RVCORESimTop.v
            -h, --help     print this help info
            -o, --output   the place you want to store your extracted verilog
                           default: $YINXING_HOME/build/extracted
  ...
  def wrapParams(args: Array[String]): (String, List[String], String, String) = {
    ...
    (paramMap("source").map(_.asInstanceOf[String]).getOrElse(env("YINXING_HOME")+"/build/RVCORESimTop.v"),
      paramMap("modules").map(_.asInstanceOf[List[String]]).getOrElse(List("IFU")),
      paramMap("output").map(_.asInstanceOf[String]).getOrElse(env("YINXING_HOME")+"/build/extracted/"),
