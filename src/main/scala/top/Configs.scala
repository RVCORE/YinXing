...
class BaseConfig(n: Int) extends Config((site, here, up) => {
  ...
  case RVCORETileKey => Seq.tabulate(n){ i => RVCORECoreParameters(HartId = i) }
  case ExportDebug => DebugAttachParams(protocols = Set(JTAG))
  case DebugModuleKey => Some(RVCOREDebugModuleParams(site(XLen)))
  case JtagDTMKey => JtagDTMKey
  case MaxHartIdBits => 2
  case EnableJtag => true.B
})

// Synthesizable minimal RVCore
...
class MinimalConfig(n: Int = 1) extends Config(
  new BaseConfig(n).alter((site, here, up) => {
    case RVCORETileKey => up(RVCORETileKey).map(
      _.copy(
        DecodeWidth = 2,
        RenameWidth = 2,
        FetchWidth = 4,
...
class MinimalSimConfig(n: Int = 1) extends Config(
  new MinimalConfig(n).alter((site, here, up) => {
    case RVCORETileKey => up(RVCORETileKey).map(_.copy(
...
class DefaultConfig(n: Int = 1) extends Config(
  new WithNKBL3(4 * 1024, inclusive = false, banks = 4, ways = 4)
    ++ new WithNKBL2(1 * 256, inclusive = false, banks = 2, alwaysReleaseData = true)
