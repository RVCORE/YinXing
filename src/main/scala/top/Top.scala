...
abstract class BaseRVCORESoc()(implicit p: Parameters) extends LazyModule
  with BindingScope
{
...
class RVCORETop()(implicit p: Parameters) extends BaseRVCORESoc() with HasSoCParameter
{
  ResourceBinding {
  ...
  val core_with_l2 = tiles.map(coreParams =>
    LazyModule(new RVCORETile()(p.alterPartial({
      case RVCORECoreParamsKey => coreParams
    })))
  )
...
object TopMain extends App with HasRocketChipStageUtils {
  override def main(args: Array[String]): Unit = {
    ...
    val soc = DisableMonitors(p => LazyModule(new RVCORETop()(p)))(config)
    Generator.execute(firrtlOpts, soc.module, firrtlComplier)
    ElaborationArtefacts.files.foreach{ case (extension, contents) =>
      writeOutputFile("./build", s"RVCORETop.${extension}", contents())
    }
  }
}
