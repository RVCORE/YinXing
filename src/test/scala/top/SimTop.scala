...
class SimTop(implicit p: Parameters) extends Module {
  ...
  val l_soc = LazyModule(new RVCORETop())
  ...
  if (!debugOpts.FPGAPlatform && debugOpts.EnablePerfDebug) {
    ...
    ExcitingUtils.addSource(clean, "RVCOREPERF_CLEAN")
    ExcitingUtils.addSource(dump, "RVCOREPERF_DUMP")
  }
...
object SimTop extends App {
  override def main(args: Array[String]): Unit = {
    // Keep this the same as TopMain except that SimTop is used here instead of RVCORETop
