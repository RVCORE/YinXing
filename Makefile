...
DEBUG_ARGS   = --enable-difftest  --full-stacktrace
ifeq ($(RELEASE),1)
...
help:
	mill -i YinXing.runMain $(FPGATOP) --help

$(TOP_V): $(SCALA_FILE)
	mkdir -p $(@D)
	$(TIME_CMD) mill -i YinXing.runMain $(FPGATOP) -td $(@D)  \
		--config $(CONFIG)                                        \
		$(FPGA_MEM_ARGS)                                          \
		--num-cores $(NUM_CORES)                                  \
...
ifeq ($(MFC),1)
	...
	@cat .__head__ .__diff__ > $(BUILD_DIR)/git_diff.log
	@cp ./src/main/scala/rvcore/Parameters.scala $(BUILD_DIR)/
	@cp ./src/main/scala/top/Configs.scala $(BUILD_DIR)/
	@realpath $(BUILD_DIR)/RVCORETop.v >> $(BUILD_DIR)/firrtl_black_box_resource_files.f
	@mv .__out__ $@
...
$(SIM_TOP_V): $(SCALA_FILE) $(TEST_FILE)
	...
	$(TIME_CMD) mill -i YinXing.test.runMain $(SIMTOP) -td $(@D)  \
		--config $(CONFIG)                                            \
		$(SIM_MEM_ARGS)                                               \
		--num-cores $(NUM_CORES)                                      \
		$(SIM_ARGS) $(FC_ARGS)
...
emu:
	$(MAKE) -C ./difftest emu SIM_TOP=SimTop DESIGN_DIR=$(YINXING_HOME) NUM_CORES=$(NUM_CORES)

emu-run:
	$(MAKE) -C ./difftest emu-run SIM_TOP=SimTop DESIGN_DIR=$(YINXING_HOME) NUM_CORES=$(NUM_CORES)

# vcs simulation
simv:
	$(MAKE) -C ./difftest simv SIM_TOP=SimTop DESIGN_DIR=$(YINXING_HOME) NUM_CORES=$(NUM_CORES)
